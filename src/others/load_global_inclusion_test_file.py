...
class GlobalInclusionFileLoad(object):
    ...
    def load_***ARANGO_DB_NAME***_terms_with_datasource(self, df: pd.DataFrame):
        query = """
        with $dict.rows as rows unwind rows as row
    ...
    def load_***ARANGO_DB_NAME***_terms_without_datasource(self, df: pd.DataFrame):
        query = """
        with $dict.rows as rows unwind rows as row
...
def add_global_inclusion_test_file(file, database):
    ...
    logging.info('add ***ARANGO_DB_NAME*** terms')
    df_ll = df[df['domain'] == 'Lifelike']
    loader.load_***ARANGO_DB_NAME***_terms_with_datasource(df_ll[df_ll['data_source'].str.len() > 0])
    loader.load_***ARANGO_DB_NAME***_terms_without_datasource(df_ll[df_ll['data_source'].str.len() == 0])
