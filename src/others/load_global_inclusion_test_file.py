...
def load_***ARANGO_DB_NAME***_terms_with_datasource(database:Database, df:pd.DataFrame):
    query = """
    with $dict.rows as rows unwind rows as row
...
def load_***ARANGO_DB_NAME***_terms_without_datasource(database:Database, df:pd.DataFrame):
    query = """
    with $dict.rows as rows unwind rows as row
