...
class MeshParser(BaseParser):
    ...
    def import_mesh_rdf(self, meshdatabase:Database):
        ...
        :param meshdatabase: neo4j database for mesh terms (not ***ARANGO_DB_NAME***)
    ...
    def load_data_to_neo4j(self, from_database: Database, to_database:Database):
        """
        Mesh data was loaded into mesh_database. Need to query the data, and load into ***ARANGO_DB_NAME*** database
        :param to_database: the ***ARANGO_DB_NAME*** database
...
def main():
    ***ARANGO_DB_NAME***_db = get_database()
    parser = MeshParser()
    # create a database 'meshdb' in neo4j for mesh rdf data
    meshdb = 'meshdb'
    query = f"CREATE OR REPLACE DATABASE {meshdb}"
    ***ARANGO_DB_NAME***_db.run_query(query)

    mesh_db = get_database()
    mesh_db.dbname = meshdb
    parser.import_mesh_rdf(mesh_db)

    parser.load_data_to_neo4j(mesh_db, ***ARANGO_DB_NAME***_db)
    parser.remove_synonyms_with_comma(***ARANGO_DB_NAME***_db)
    parser.set_data_source(***ARANGO_DB_NAME***_db)
    add_annotation_entity_labels(***ARANGO_DB_NAME***_db)
    mesh_db.close()
    # can drop meshdb now
    write_mesh_annotation_files(***ARANGO_DB_NAME***_db, parser.output_dir)
    ***ARANGO_DB_NAME***_db.close()
