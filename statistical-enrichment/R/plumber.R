    ...
    tryCatch(
      {
        ...
        tryCatch(
          {
            res$body <- jsonlite::toJSON(
              fisher.enrich(genes, annotations, q.threshold),
              auto_unbox = TRUE
            )
            res$status <- 200
          },
...
#* @param annotations annotation,id
#* @param GOIs A file
#* @param qThreshold A file
generate_json_service(
  function(data) {
    annotations <- data$annotations
    GOIs <- data$GOIs
    q.threshold <- data$qThreshold
    return(annotations,GOIs,q.threshold)
  },
  binom.enrich
)
