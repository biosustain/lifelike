...
class KeggDataLoader(BaseDataLoader):
    ...
    def load_pathway_nodes(self, database):
        ...
        database.load_csv_file(
            query,
            os.path.join(self.output_dir, PATHWAY_FILE),
            dtype={PROP_ID: str}
            )
    ...
    def get_load_gene2ncbi_query(cls):
        return get_create_relationships_query(
            NODE_KEGG, PROP_ID, PROP_ID, NODE_GENE, PROP_ID, PROP_GENE_ID,
            REL_IS
            )
    ...
    def load_ko2pathway_rels(self, database):
        ...
        database.load_csv_file(
            query,
            os.path.join(self.output_dir, KO2PATHWAY_FILE),
            chunksize=5000
            )
    ...
    def get_load_genome2pathway_query(cls):
        return get_create_relationships_query(
            NODE_GENOME, PROP_ID, 'genome', NODE_PATHWAY, PROP_ID, 'pathway',
            REL_HAS_PATHWAY
            )
    ...
    def load_genome2pathway_rels(self, database):
        ...
        database.load_csv_file(
            query, os.path.join(self.output_dir, GENOME2PATHWAY_FILE), sep='\t',
