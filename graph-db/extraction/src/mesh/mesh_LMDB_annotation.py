...
def _write_entity_list_for_LMDB(
    entity_node_label: str, database: Database, output_dir: str
):
...
def write_mesh_annotation_files(database, output_dir):
    _write_entity_list_for_LMDB(
        NODE_DISEASE, database, f'{output_dir}/processed/disease'
    )
    _write_entity_list_for_LMDB(NODE_FOOD, database, f'{output_dir}/processed/food')
    _write_entity_list_for_LMDB(
        NODE_ANATOMY, database, f'{output_dir}/processed/anatomy'
    )
    _write_entity_list_for_LMDB(
        NODE_PHENOMENA, database, f'{output_dir}/processed/phenomena'
    )
