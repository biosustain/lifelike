...
class MeshParser(BaseParser):
    ...
    def _load_treenumber(self, db):
        ...
        self.logger.info(f'len of df: {len(df)}')
        df[PROP_ID] = df[PROP_ID].str.replace('[OBSOLETE]', '', regex=False).str.strip()
        df[PROP_PARENT_ID] = df[PROP_PARENT_ID].str.replace(
            '[OBSOLETE]',
            '',
            regex=False
            ).str.strip()
...
def main(args):
    ...
    for filename in [
        MESH_CHEMICAL_FILE, MESH_CHEMICAL_TOPICALDESC_REL_FILE, MESH_DISEASE_FILE,
        MESH_DISEASE_TOPICALDESC_REL_FILE,
        MESH_SYNONYM_REL_FILE, MESH_TOPICALDESC_FILE, MESH_TREENUMBER_FILE,
        MESH_TREENUMBER_PARENT_REL_FILE, MESH_TREENUMBER_TOPICALDESC_REL_FILE
    ]:
