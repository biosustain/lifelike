...
class MeshParser(BaseParser):
    ...
    def _load_treenumber(self, db):
        ...
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + MESH_TREENUMBER_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')

        self.logger.info('node treenumber has_parent relationship')
        df = db.get_data(
            f'MATCH (t:TreeNumber)-[:parentTreeNumber]->(p:TreeNumber) RETURN t.label AS {PROP_ID}, p.label AS {PROP_PARENT_ID}'
        )
        self.logger.info(f'len of df: {len(df)}')
        df[PROP_ID] = df[PROP_ID].str.replace('[OBSOLETE]', '', regex=False).str.strip()
        df[PROP_PARENT_ID] = (
            df[PROP_PARENT_ID].str.replace('[OBSOLETE]', '', regex=False).str.strip()
        )
        ...
        print(query)
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + MESH_TREENUMBER_PARENT_REL_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')
    ...
    def _load_topical_descriptor(self, db):
        ...
        df = db.get_data(
            f'MATCH (d:TopicalDescriptor) RETURN d.identifier AS {PROP_ID}, d.label AS {PROP_NAME}'
        )
        self.logger.info(f'len of df: {len(df)}')
        df[PROP_OBSOLETE] = df[PROP_NAME].str.startswith('[OBSOLETE]').astype(int)
        df[PROP_NAME] = (
            df[PROP_NAME].str.replace('[OBSOLETE]', '', regex=False).str.strip()
        )
        query = get_create_update_nodes_query(
            NODE_MESH,
            PROP_ID,
        ...
        print(query)
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + MESH_TOPICALDESC_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')

        self.logger.info('map mesh to tree-number')
        df = db.get_data(
            f'MATCH (t:TreeNumber)-[]-(d:TopicalDescriptor) RETURN d.identifier AS {PROP_ID}, t.label AS treenumber'
        )
        self.logger.info(f'len of df: {len(df)}')
        df['treenumber'] = (
            df['treenumber'].str.replace('[OBSOLETE]', '', regex=False).str.strip()
        )
        query = get_create_relationships_query(
            NODE_MESH,
            PROP_ID,
    ...
    def _load_chemical(self, db):
        ...
        df = db.get_data(
            f'MATCH (n:SCR_Chemical) RETURN n.identifier AS {PROP_ID}, n.label AS {PROP_NAME}'
        )
        self.logger.info(f'len of df: {len(df)}')
        df[PROP_OBSOLETE] = df[PROP_NAME].str.startswith('[OBSOLETE]').astype(int)
        df[PROP_NAME] = (
        ...
        print(query)
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + MESH_CHEMICAL_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')
        ...
        print(query)
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + MESH_CHEMICAL_TOPICALDESC_REL_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')
    ...
    def _load_disease(self, db):
        ...
        df = db.get_data(
            f'MATCH (n:SCR_Disease) RETURN n.identifier AS {PROP_ID}, n.label AS {PROP_NAME}'
        )
        df[PROP_OBSOLETE] = df[PROP_NAME].str.startswith('[OBSOLETE]').astype(int)
        df[PROP_NAME] = (
            df[PROP_NAME].str.replace('[OBSOLETE]', '', regex=False).str.strip()
        ...
        print(query)
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + MESH_DISEASE_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')
        ...
        print(query)
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + MESH_DISEASE_TOPICALDESC_REL_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')
    ...
    def _load_synonym(self, db):
        ...
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + MESH_SYNONYM_REL_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')
