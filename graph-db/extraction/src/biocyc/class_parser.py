...
class ClassParser(BaseDataFileParser):
    ...
    def parse_and_write_data_files(self):
        self.nodes = [
            n
            for n in self.nodes
            if not n.get_attribute(PROP_BIOCYC_ID).startswith(('GO:', 'TAX-', 'ORG-'))
        ]
        for node in self.nodes:
            ...
            node.edges = [
                edge
                for edge in edges_with_type_of
                if edge.dest.get_attribute(PROP_BIOCYC_ID) != FRAMES
                and edge.source.get_attribute(PROP_BIOCYC_ID) != FRAMES
            ]
