...
NODE_ATTRS = [PROP_ID, PROP_NAME, PROP_RANK, PROP_CATEGORY]
REL_ATTRS = [PROP_FROM_ID, PROP_TO_ID]
...
class TaxonomyParser(BaseParser):
    ...
    def parse_files(self) -> dict:
        ...
        with zipfile.ZipFile(self.zip_file) as zp:
            # get all tax_id and name, set top class nodes
            ...
            # read nodes file for rank and hierarchy
        ...
        # set up top node class as top class tax name.
    ...
    def parse_and_write_data_files(self, zip_outfile):
        ...
        tax_file = open(os.path.join(self.output_dir, TAXONOMY_FILE), 'w')
        tax_rel_file = open(os.path.join(self.output_dir, TAXONONYM_REL_FILE), 'w')
        synonym_file = open(os.path.join(self.output_dir, TAXONOMY_SYNONYM_FILE), 'w')
        tax_file.write('\t'.join(NODE_ATTRS) + '\n')
        tax_rel_file.write('\t'.join(REL_ATTRS) + '\n')
        synonym_file.write('\t'.join(SYNONYM_NODE_ATTRS) + '\n')
        # only write nodes with top class set.  Other nodes are not organisms (e.g. sequences)
        ...
        tax_rel_file.close()
        synonym_file.close()
        outfiles = [TAXONOMY_FILE, TAXONONYM_REL_FILE, TAXONOMY_SYNONYM_FILE]
        self.zip_output_files(outfiles, zip_outfile)


def main():
    parser = TaxonomyParser()
    parser.parse_and_write_data_files('taxonomy_data_220520.zip')
