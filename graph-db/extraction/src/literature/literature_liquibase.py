...
class LiteratureChangeLog(ChangeLog):
    ...
    def load_literature_chemical_mapped_rels(self):
        ...
        'MERGE (n)-[:MAPPED_TO]->(c:db_MESH {eid:split(n.eid, ":")[1]}) FOREACH (item IN CASE WHEN NOT "Chemical" IN labels(c) THEN [1] ELSE [] END | SET c:Chemical)',
        {batchSize:10000});
        CALL apoc.periodic.iterate(
        'MATCH (n:db_Literature:LiteratureChemical) WHERE n.eid CONTAINS "CHEBI:" RETURN n',
        'MERGE (n)-[:MAPPED_TO]->(c:db_CHEBI {eid:split(n.eid, ":")[1]}) FOREACH (item IN CASE WHEN NOT "Chemical" IN labels(c) THEN [1] ELSE [] END | SET c:Chemical)',
        {batchSize:10000});
        """
    ...
    def load_literature_disease_mapped_rels(self):
        ...
        'MERGE (n)-[:MAPPED_TO]->(d:db_MESH {eid:split(n.eid, ":")[1]}) FOREACH (item IN CASE WHEN NOT "Disease" IN labels(d) THEN [1] ELSE [] END | SET d:Disease)',
        {batchSize:10000});
        CALL apoc.periodic.iterate(
