...
class KeggDataLoader(BaseDataLoader):
    ...
    def get_load_gene_query(cls):
        ...
        query = get_create_update_nodes_query(
            NODE_KEGG, PROP_ID, cols, [NODE_GENE], datasource=DB_KEGG
        )
        return query
    ...
    def load_gene_nodes(self, database):
        ...
        database.load_csv_file(
            query,
            os.path.join(self.output_dir, GENE_FILE),
            sep='\t',
            dtype={PROP_GENE_ID: str},
            chunksize=5000,
        )

    @classmethod
    def get_load_geneome_query(cls):
        return get_create_update_nodes_query(
            NODE_GENOME,
            PROP_ID,
            [],
            [NODE_KEGG],
            datasource=DB_KEGG,
            original_entity_type=NODE_GENOME,
        )
    ...
    def load_genome_nodes(self, database):
        ...
        database.load_csv_file(
            query, os.path.join(self.output_dir, GENOME_FILE), sep='\t'
        )
    ...
    def get_load_ko_query(cls):
        ...
        query = get_create_update_nodes_query(
            NODE_KEGG, PROP_ID, cols, [NODE_KO], datasource=DB_KEGG
        )
        return query
    ...
    def get_load_pathway_query(cls):
        ...
        query = get_create_update_nodes_query(
            NODE_KEGG, PROP_ID, cols, [NODE_PATHWAY], DB_KEGG, NODE_PATHWAY
        )
        return query
    ...
    def load_pathway_nodes(self, database):
        ...
        database.load_csv_file(
            query, os.path.join(self.output_dir, PATHWAY_FILE), dtype={PROP_ID: str}
        )

    @classmethod
    def get_load_gene2ko_query(cls):
        return get_create_relationships_query(
            NODE_KEGG, PROP_ID, 'gene', NODE_KO, PROP_ID, 'ko', 'HAS_KO'
        )
    ...
    def load_gene2ko_rels(self, database):
        ...
        database.load_csv_file(
            query, os.path.join(self.output_dir, GENE2KO_FILE), chunksize=5000
        )

    @classmethod
    def get_load_gene2ncbi_query(cls):
        return get_create_relationships_query(
            NODE_KEGG, PROP_ID, PROP_ID, NODE_GENE, PROP_ID, PROP_GENE_ID, REL_IS
        )
    ...
    def get_load_ko2pathway_query(cls):
        return get_create_relationships_query(
            NODE_KO, PROP_ID, 'ko', NODE_PATHWAY, PROP_ID, 'pathway', 'IN_PATHWAY'
        )
    ...
    def load_ko2pathway_rels(self, database):
        ...
        database.load_csv_file(
            query, os.path.join(self.output_dir, KO2PATHWAY_FILE), chunksize=5000
        )
    ...
    def load_genome2pathway_rels(self, database):
        ...
        database.load_csv_file(
            query,
            os.path.join(self.output_dir, GENOME2PATHWAY_FILE),
            sep='\t',
            dtype={'pathway': str},
            chunksize=5000,
        )
