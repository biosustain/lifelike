...
class GeneParser(BaseParser):
    ...
    def _load_bioinfo_to_neo4j(self):
        ...
        geneinfo_chunks = pd.read_csv(
            self.gene_info_file, sep='\t', chunksize=10000, usecols=gene_info_cols
        )

        outfile = os.path.join(self.output_dir, self.file_prefix + NCBI_GENE_FILE)
        syn_outfile = os.path.join(
            self.output_dir, self.file_prefix + NCBI_GENE_SYNONYM_FILE
        )
        ...
        for i, chunk in enumerate(geneinfo_chunks):
            ...
            df_syn = (
                df_syn.reset_index()
                .rename(columns={0: 'synonym'})
                .loc[:, [PROP_ID, 'synonym']]
            )
            # ignore single letter synonym, and synonyms without letters
            df_syn = df_syn[
                df_syn['synonym'].str.len()
                > 1 & df_syn['synonym'].str.contains('[a-zA-Z]')
            ]
    ...
    def _load_gene2go_to_neo4j(self):
        chunks = pd.read_csv(
            self.gene2go_file, sep='\t', chunksize=10000, usecols=['GeneID', 'GO_ID']
        )
