...
class RegulonDbParser(BaseParser):
    ...
    def process_file(self, filename: str, node_prop_map: dict):
        ...
        df = pd.read_csv(
            data_file, delimiter='\t', names=df_headers, skiprows=skiplines
        )
        df = df.fillna('')
    ...
    def create_nodes(self, node_label, node_prop_map: dict):
        ...
        query = get_create_update_nodes_query(
            NODE_REGULONDB, PROP_REGULONDB_ID, properties, [node_label]
        )
        print(query)

    def create_edges(
        self, rel_type: str, start_node_id_col, end_node_id_col, rel_property_map=None
    ):
        rel_properties = []
        if rel_property_map:
    ...
    def load_genes(self):
        ...
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + REGULON_GENE_FILE
        )
    ...
    def load_operons(self):
        ...
        self._clean_characters(df, PROP_NAME)
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + REGULON_OPERON_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')
    ...
    def load_gene_products(self):
        ...
        self._clean_characters(df, PROP_NAME)
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + REGULON_PRODUCT_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')
    ...
    def load_promoters(self):
        ...
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + REGULON_PROMOTER_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')
    ...
    def load_terminators(self):
        ...
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + REGULON_TERMINATOR_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')
    ...
    def load_transcription_factors(self):
        ...
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + REGULON_TRANSCRIPTION_FACTOR_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')
    ...
    def load_transunits(self):
        ...
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + REGULON_TRANSCRIPTION_UNIT_FILE
        )
        self._clean_characters(df, PROP_NAME)
        ...
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + REGULON_PROMOTOR_TRANSUNIT_REL_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')
        ...
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + REGULON_OPERON_TRANSUNIT_REL_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')

    def map_gene_product_link(self):
        """associate gene with product, return list of edges"""
        gene_product_map = {'GENE_ID': 'gene_id', 'PRODUCT_ID': 'product_id'}
        ...
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + REGULON_GENE_PRODUCT_REL_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')
    ...
    def map_regulon_tf_link(self):
        ...
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + REGULON_TRANSFACTOR_REL_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')
    ...
    def map_tu_gene_link(self):
        ...
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + REGULON_GENE_TRANSUNIT_REL_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')
    ...
    def map_genetic_network(self):
        ...
        outfile = os.path.join(
            self.output_dir, f'{self.file_prefix}' + REGULON_REGULATES_REL_FILE
        )
        df.to_csv(outfile, index=False, sep='\t')

    def map_regulon_promoter_link(self):
        self.logger.info('Associate regulon with promoter')
        attr_map1 = {'REGULON_FUNCTION_ID': 'function_id', 'PROMOTER_ID': 'promoter_id'}
        attr_map2 = {
    ...
    def write_gene2bnumber(self):
        with open(
            os.path.join(self.download_dir, 'object_synonym.txt'), 'r'
        ) as f, open(
            os.path.join(
                self.output_dir, f'{self.file_prefix}' + REGULON_NCBI_GENE_REL_FILE
            ),
            'w',
        ) as outfile:
