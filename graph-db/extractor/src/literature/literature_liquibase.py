...
class LiteratureChangeLog(ChangeLog):
    ...
    def load_literature_model(self):
        for filename, (entity1_type, entity2_type) in [
            ...
            comment = (
                f'Split creation of {entity1_type}-{entity2_type} nodes and assocation from snippet creation; seem to be faster instead of one giant cypher'
            ...
            query = LiteratureDataParser.get_create_literature_query(
    ...
    def load_literature_snippet_pub_rels(self):
        for filename, (entity1_type, entity2_type) in [
            ...
            if self.id_prefix:
                id = f'{self.id_prefix} {id}'
            comment = (
                f'Link snippets for {entity1_type}-{entity2_type} to publication. \nNeed to include properties in INDICATES relationship merge '
    ...
    def create_indexes(self):
        ...
        queries.append(
            get_create_constraint_query(
                NODE_ASSOCIATION, PROP_ID, 'constraint_association_id'
            )
            + ';'
        )
        queries.append(
            get_create_constraint_query(NODE_SNIPPET, PROP_ID, 'constraint_snippet_id')
            + ';'
        )
        queries.append(
            get_create_constraint_query(
                NODE_LITERATURE_ENTITY, PROP_ID, 'constraint_literatureentity_id'
            )
            + ';'
        )
        queries.append(
            get_create_constraint_query(
                NODE_LITERATURE_DISEASE, PROP_ID, 'constraint_literaturedisease_id'
            )
            + ';'
        )
        queries.append(
            get_create_constraint_query(
                NODE_LITERATURE_CHEMICAL, PROP_ID, 'constraint_literaturechemical_id'
            )
            + ';'
        )
        queries.append(
            get_create_constraint_query(
                NODE_LITERATURE_GENE, PROP_ID, 'constraint_literaturegene_id'
            )
            + ';'
        )
        queries.append(
            get_create_constraint_query(
                NODE_PUBLICATION, PROP_PUBLICATION_ID, 'constraint_publication_pmid'
            )
            + ';'
        )
        return queries
