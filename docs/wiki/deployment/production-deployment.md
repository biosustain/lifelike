...
```bash
gsutil cp gs://kg-secrets/lifelike-accounts.txt .
```
4. Superuser for lifelike.bio
```bash
gsutil cp gs://kg-secrets/lifelike-accounts.txt .
```
...
http:
  routers:
    ...
    lifelike-demo:
      entryPoints:
        - websecure
      rule: "Host(`demo.lifelike.bio`)"
      service: lifelike-demo-service
    tls:
      certResolver: "le"
      domains:
        certResolver: "le"
        domains:
          - main: "demo.lifelike.bio"
    lifelike-qa:
      entryPoints:
        - websecure
