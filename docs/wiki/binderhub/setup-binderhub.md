...
You can name the IP address whatever you want, but use something descriptive like "lifelike-binderhub-proxy". If you'd like a more in-depth explanation of how to reserve a static IP, please follow the [official guide](https://cloud.google.com/compute/docs/ip-addresses/reserve-static-external-ip-address).
...
```bash
helm upgrade lifelike-binderhub jupyterhub/binderhub --version=1.0.0-0.dev.git.3128.h52ffd88 --namespace=lifelike-binderhub -f config-prod.yaml
```
