...
CREATE
OR REPLACE FUNCTION tsq_append_current_token(state tsq_state)
RETURNS tsq_state AS $$
BEGIN
    IF
state.current_token != '' THEN
...
CREATE
OR REPLACE FUNCTION tsq_tokenize_character(state tsq_state)
RETURNS tsq_state AS $$
BEGIN
    IF
state.current_char = '(' THEN
        state.tokens := array_append(state.tokens, '(');
        state.parentheses_stack
:= state.parentheses_stack + 1;
        state
:= tsq_append_current_token(state);
    ELSIF
state.current_char = ')' THEN
        IF (state.parentheses_stack > 0 AND state.current_token != '') THEN
            state := tsq_append_current_token(state);
            state.tokens
:= array_append(state.tokens, ')');
            state.parentheses_stack
:= state.parentheses_stack - 1;
END IF;
    ELSIF
...
CREATE
OR REPLACE FUNCTION tsq_tokenize(search_query text) RETURNS text[] AS $$
DECLARE
state tsq_state;
BEGIN
SELECT search_query::text AS search_query, 0::int AS parentheses_stack, 0 AS skip_for,
       ''::text AS current_token, 0 AS current_index,
       ''::text AS current_char, ''::text AS previous_char, '{}'::text[] AS tokens
INTO state;
...
:= tsq_append_current_token(state);
    ...
    state.tokens
:= array_nremove(state.tokens, '(', -state.parentheses_stack);
...
CREATE
OR REPLACE FUNCTION tsq_process_tokens(config regconfig, tokens text[])
RETURNS tsquery AS $$
DECLARE
result_query text;
    previous_value
...
END IF;
        ...
        IF left
(value, 1) = '"' AND right(value, 1) = '"' THEN
...
CREATE
OR REPLACE FUNCTION tsq_process_tokens(tokens text[])
RETURNS tsquery AS $$
...
CREATE
OR REPLACE FUNCTION tsq_parse(config regconfig, search_query text)
RETURNS tsquery AS $$
...
CREATE
OR REPLACE FUNCTION tsq_parse(config text, search_query text)
RETURNS tsquery AS $$
...
CREATE
OR REPLACE FUNCTION tsq_parse(search_query text) RETURNS tsquery AS $$
...
CREATE
OR REPLACE FUNCTION array_nremove(anyarray, anyelement, int)
RETURNS ANYARRAY AS $$
...
SELECT COALESCE((SELECT array_agg(value)
                 FROM unnest($1) WITH ORDINALITY AS t(value, index)
                 WHERE index NOT IN (SELECT position FROM replaced_positions)), $1[1:0]);
$$
LANGUAGE SQL IMMUTABLE;

CREATE
OR REPLACE FUNCTION json2text(json)
  RETURNS TEXT AS
$func$
SELECT string_agg(CAST(elements - > 'display_name' as VARCHAR), ',')
FROM json_array_elements($1 - > 'nodes') elements $func$ LANGUAGE SQL IMMUTABLE STRICT;
