...
def upgrade():
    ...
    op.create_index(
        op.f('ix_files_content_checksum_sha256'),
        'files_content',
        ['checksum_sha256'],
        unique=True,
    )
    ...
    for file_id, raw_file in files:
        ...
        existing_content_result = conn.execute(
        ...
        if existing_content_result:
            ...
            content_id = conn.execute(
                t_files_content.insert().values(
                    raw_file=raw_file, checksum_sha256=checksum_sha256
                )
    ...
    op.alter_column(
        'files', 'content_id', existing_type=postgresql.INTEGER(), nullable=False
    )
    op.alter_column(
        'files_content', 'raw_file', existing_type=postgresql.BYTEA(), nullable=False
    )
...
def downgrade():
    ...
    for file_id, raw_file in files:
        conn.execute(
            t_files.update().where(t_files.c.id == file_id).values(raw_file=raw_file)
        )
