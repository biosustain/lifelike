...
def upgrade():
    ...
    op.add_column('projects', sa.Column('***ARANGO_USERNAME***_id', sa.Integer(), nullable=True))  # Remove nullable later
    op.create_index(op.f('ix_projects_***ARANGO_USERNAME***_id'), 'projects', ['***ARANGO_USERNAME***_id'], unique=False)
    op.create_foreign_key(op.f('fk_projects_***ARANGO_USERNAME***_id_files'), 'projects', 'files', ['***ARANGO_USERNAME***_id'], ['id'])
    ...
    dummy_dir_owner_id = None  # The user ID to 'own' directories for projects without a ***ARANGO_USERNAME*** dir

    logger.info("Making sure all projects have a ***ARANGO_USERNAME*** directory...")
    for project in iter_query(session.query(t_project).filter(t_project.c.***ARANGO_USERNAME***_id.is_(None)),
        ...
        if not dummy_dir_owner_id:
            ...
            logger.info("Created a dummy user (_migration_***ARANGO_USERNAME***_dir, #%s) to own the directories "
                        "of projects without a ***ARANGO_USERNAME*** directory", dummy_dir_owner_id)
