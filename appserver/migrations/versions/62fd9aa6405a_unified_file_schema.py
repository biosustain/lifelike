...
def upgrade():
    ...
    op.add_column(
        'projects', sa.Column('root_id', sa.Integer(), nullable=True)
    )  # Remove nullable later
    op.create_index(op.f('ix_projects_root_id'), 'projects', ['root_id'], unique=False)
    op.create_foreign_key(
        op.f('fk_projects_root_id_files'), 'projects', 'files', ['root_id'], ['id']
    )
    ...
    session.execute(
        t_file.update()
        .values(mime_type='vnd.lifelike.document/enrichment-table')
    ...
    dummy_dir_owner_id = (
        None  # The user ID to 'own' directories for projects without a root dir
    )

    logger.info("Making sure all projects have a root directory...")
    for project in iter_query(
        session.query(t_project).filter(t_project.c.root_id.is_(None)),
        ...
        if not dummy_dir_owner_id:
            ...
            logger.info(
                "Created a dummy user (_migration_root_dir, #%s) to own the directories "
                "of projects without a root directory",
                dummy_dir_owner_id,
            )
