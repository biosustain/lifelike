...
def upgrade():
    ...
    if os.environ.get("USE_UNIFIED_FILE_SCHEMA_MIGRATION_SAMPLE"):
        logger.info(
            "Inserting sample data for testing the migration "
    ...
    op.create_foreign_key(
        op.f("fk_files_parent_id_files"), "files", "files", ["parent_id"], ["id"]
    )
    ...
    op.add_column(
        "files", sa.Column("deletion_date", sa.TIMESTAMP(timezone=True), nullable=True)
    )
    op.add_column(
        "files", sa.Column("recycling_date", sa.TIMESTAMP(timezone=True), nullable=True)
    )
    ...
    op.create_foreign_key(
        op.f("fk_files_creator_id_appuser"), "files", "appuser", ["creator_id"], ["id"]
    )
    op.create_foreign_key(
        op.f("fk_files_modifier_id_appuser"),
        "files",
        "appuser",
        ["modifier_id"],
        ["id"],
    ...
    op.create_foreign_key(
        op.f("fk_files_deleter_id_appuser"), "files", "appuser", ["deleter_id"], ["id"]
    )
    op.create_foreign_key(
        op.f("fk_files_recycler_id_appuser"),
        "files",
        "appuser",
        ["recycler_id"],
        ["id"],
    ...
    # Add ***ARANGO_USERNAME***_id column to the projects table
    op.add_column(
        "projects", sa.Column("***ARANGO_USERNAME***_id", sa.Integer(), nullable=True)
    )  # Remove nullable later
    op.create_index(op.f("ix_projects_***ARANGO_USERNAME***_id"), "projects", ["***ARANGO_USERNAME***_id"], unique=False)
    op.create_foreign_key(
        op.f("fk_projects_***ARANGO_USERNAME***_id_files"), "projects", "files", ["***ARANGO_USERNAME***_id"], ["id"]
    )
    ...
    op.create_foreign_key(
        op.f("fk_projects_modifier_id_appuser"),
        "projects",
        "appuser",
        ["modifier_id"],
        ["id"],
    ...
    op.create_foreign_key(
        op.f("fk_projects_creator_id_appuser"),
        "projects",
        "appuser",
        ["creator_id"],
        ["id"],
    ...
    op.create_foreign_key(
        op.f("fk_projects_deleter_id_appuser"),
        "projects",
        "appuser",
        ["deleter_id"],
        ["id"],
    ...
    for user in iter_query(
        session.query(t_app_user), batch_size=DEFAULT_QUERY_BATCH_SIZE
    ):
        user = user._asdict()
        session.execute(
            t_app_user.update()
            .values(hash_id=create_hash_id())
            .where(t_app_user.c.id == user["id"])
        )

    # Make nullable=False
    op.alter_column(
        "appuser", "hash_id", existing_type=sa.String(length=36), nullable=False
    )
    ...
    for project in iter_query(
        session.query(t_project), batch_size=DEFAULT_QUERY_BATCH_SIZE
    ):
    ...
    # Make nullable=False
    op.alter_column(
        "projects", "hash_id", existing_type=sa.String(length=36), nullable=False
    )
    ...
    session.execute(
        t_file.update()
        .values(mime_type="vnd.***ARANGO_DB_NAME***.document/enrichment-table")
    ...
    for map in iter_query(
        session.query(t_map, t_directory.c.projects_id).join(
            t_directory, t_directory.c.id == t_map.c.dir_id
        ),
        batch_size=CONTENT_QUERY_BATCH_SIZE,
    ):
        map = map._asdict()

        content_id = get_or_create_content_id(
            session, json.dumps(map["graph"]).encode("utf-8"), map["creation_date"]
        )

        new_id = session.execute(
            t_file.insert().values(
                hash_id=map["hash_id"],
                parent_id=None,
    ...
    # ========================================
    # Copy directories to the file table and update project ***ARANGO_USERNAME***_dir
    # ========================================
    ...
    for dir in iter_query(
        session.query(t_directory), batch_size=DEFAULT_QUERY_BATCH_SIZE
    ):
        dir = dir._asdict()
    ...
    dummy_dir_owner_id = (
        None  # The user ID to 'own' directories for projects without a ***ARANGO_USERNAME*** dir
    )

    logger.info("Making sure all projects have a ***ARANGO_USERNAME*** directory...")
    for project in iter_query(
        session.query(t_project).filter(t_project.c.***ARANGO_USERNAME***_id.is_(None)),
        ...
        if not dummy_dir_owner_id:
            dummy_dir_owner_id = session.execute(
                t_app_user.insert().values(
                    hash_id=create_hash_id(),
                    username="_migration_***ARANGO_USERNAME***_dir",
                    email="migration_***ARANGO_USERNAME***_dir@migration.***ARANGO_DB_NAME***.bio",
                    first_name="System",
                    last_name="Lifelike",
                )
            ).inserted_primary_key[0]
            logger.info(
                "Created a dummy user (_migration_***ARANGO_USERNAME***_dir, #%s) to own the directories "
                "of projects without a ***ARANGO_USERNAME*** directory",
                dummy_dir_owner_id,
            )

        # Create the ***ARANGO_USERNAME*** directory
        dir_id = session.execute(
            t_file.insert().values(
                hash_id=create_hash_id(),
                parent_id=None,
                mime_type="vnd.***ARANGO_DB_NAME***.filesystem/directory",
        ...
        session.execute(
            t_project.update()
            .values(
                ***ARANGO_USERNAME***_id=dir_id,
            )
            .where(t_project.c.id == project["id"])
        )

    # Make ***ARANGO_USERNAME*** ID non-nullable
    logger.info("Making the ***ARANGO_USERNAME***_id column non-nullable...")
    op.alter_column("projects", "***ARANGO_USERNAME***_id", existing_type=sa.Integer(), nullable=False)
    ...
    for file in iter_query(
        session.query(t_file).filter(t_file.c.dir_id.isnot(None)),
        batch_size=DEFAULT_QUERY_BATCH_SIZE,
    ):
        file = file._asdict()
        ...
        logger.debug(
            "-> File(id=%s, dir_id=%s) updated with parent_id=%s",
            file["id"],
            file["dir_id"],
            new_parent_id,
        )
    ...
    op.create_table(
        ...
        sa.Column("deleter_id", sa.Integer(), nullable=True),
        sa.ForeignKeyConstraint(
            ["file_id"], ["files.id"], name=op.f("fk_file_version_file_id_files")
        ),
        sa.ForeignKeyConstraint(
            ["content_id"],
            ["files_content.id"],
        ...
        sa.ForeignKeyConstraint(
            ["user_id"],
            ["appuser.id"],
            name=op.f("fk_file_version_user_id_appuser"),
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_file_version")),
    )
    op.create_index(
        op.f("ix_file_version_file_id"), "file_version", ["file_id"], unique=False
    )
    op.create_index(
        op.f("ix_file_version_content_id"), "file_version", ["content_id"], unique=False
    )
    op.create_index(
        op.f("ix_file_version_user_id"), "file_version", ["user_id"], unique=False
    )
    op.create_unique_constraint(
        op.f("uq_file_version_hash_id"), "file_version", ["hash_id"]
    )
    ...
    for backup in iter_query(
        session.query(t_map_version), batch_size=CONTENT_QUERY_BATCH_SIZE
    ):
        ...
        logger.debug(
            "-> ProjectVersion(id=%s, project_id=%s) copied "
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["file_id"], ["files.id"], name=op.f("fk_file_backup_file_id_files")
        ),
        sa.ForeignKeyConstraint(
            ["modifier_id"],
            ["appuser.id"],
        ...
        sa.ForeignKeyConstraint(
            ["user_id"],
            ["appuser.id"],
            name=op.f("fk_file_backup_user_id_appuser"),
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_file_backup")),
    )
    op.create_index(
        op.f("ix_file_backup_file_id"), "file_backup", ["file_id"], unique=False
    )
    op.create_index(
        op.f("ix_file_backup_user_id"), "file_backup", ["user_id"], unique=False
    )
    op.create_unique_constraint(
        op.f("uq_file_backup_hash_id"), "file_backup", ["hash_id"]
    )
    ...
    for backup in iter_query(
        session.query(t_map_backup), batch_size=CONTENT_QUERY_BATCH_SIZE
    ):
        backup = backup._asdict()
        ...
        logger.debug(
            "-> ProjectBackup(project_id=%s) copied "
    ...
    duplicate_sq1 = (
        session.query(
            ...
            sa.func.count()
            .over(
                partition_by=[t_file.c.filename, t_file.c.parent_id],
            )
            .label("count"),
            sa.func.rank()
            .over(
    ...
    duplicate_sq3 = session.query(
        ...
        sa.case(
            [(duplicate_sq2.c.index == 1, duplicate_sq2.c.filename)],
            else_=sa.func.concat(
                duplicate_sq2.c.filename, " (", duplicate_sq2.c.index, ")"
            ),
        ).label("new_filename"),
    ).subquery()
...
def insert_sample_data(session):
    ...
    session.execute(
        t_projects.insert().values(
            project_name="Test", users=[], creation_date=datetime.now()
        )
    )

    # No ***ARANGO_USERNAME*** DIR
    session.execute(
        t_projects.insert().values(
            project_name="No Root Dir", users=[], creation_date=datetime.now()
        )
    )
