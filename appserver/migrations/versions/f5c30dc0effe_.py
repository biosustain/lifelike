    ...
    for file_id, user_id in files:
        if user_id is None:
            ...
            if not migration_error_user_id:
                migration_error_user_result = conn.execute(
                    db.select([t_user.c.id]).where(
                        t_user.c.username == "_unknown_uploader_"
                    )
                ).fetchone()
                ...
                if migration_error_user_result:
                    ...
                    migration_error_user_id = conn.execute(
                        t_user.insert().values(
                            username="_unknown_uploader_",
        ...
        conn.execute(
            t_files.update().where(t_files.c.id == file_id).values(user_id=user_id)
        )

    op.alter_column("files", "user_id", existing_type=db.INTEGER(), nullable=False)
...
def downgrade():
    op.add_column(
        "files",
        db.Column(
            "username", db.VARCHAR(length=30), autoincrement=False, nullable=True
        ),
    )
    ...
    files = conn.execute(
        ...
        ).select_from(t_files.join(t_user, t_files.c.user_id == t_user.c.id))
    ).fetchall()

    for file_id, username in files:
        conn.execute(
            t_files.update().where(t_files.c.id == file_id).values(username=username)
        )
