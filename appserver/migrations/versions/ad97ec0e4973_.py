...
def _get_superuser_id(conxn: Connection):
    ...
    return conxn.execute(select([
        ...
        t_appuser.c.email == 'superuser@lifelike.bio'
    )).scalar()


def _create_root_file_for_master_project(conxn: Connection, superuser_id: int) -> int:
    # Create root file
...
def data_upgrades():
    ...
    master_initial_folder_id = _create_root_file_for_master_project(conxn, superuser_id)
