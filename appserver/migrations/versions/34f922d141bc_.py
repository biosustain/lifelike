...
def data_upgrades():
    ...
    users = conn.execute(sa.select([t_app_user.c.id, t_app_user.c.username])).fetchall()
    ...
    for user_id, username in users:
        ...
        (admin_role_id,) = conn.execute(
            sa.select([t_app_role.c.id]).where(t_app_role.c.name == "project-admin")
        ).fetchone()

        conn.execute(
            AccessControlPolicy.__table__.insert().values(
        ...
        (read_role_id,) = conn.execute(
            sa.select([t_app_role.c.id]).where(t_app_role.c.name == "project-read")
        ).fetchone()
        ...
        proj_ids = conn.execute(
            sa.select([t_project.c.id]).where(t_project.c.user_id == user_id)
        ).fetchall()
        ...
        file_ids = conn.execute(
            sa.select([t_files.c.id]).where(t_files.c.user_id == user_id)
        ).fetchall()
