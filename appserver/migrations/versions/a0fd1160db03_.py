...
def upgrade():
    op.create_table(
        ...
        sa.UniqueConstraint(
            'file_id', 'user_id', name='uq_starred_file_unique_user_file'
        ),
    )
    op.create_index(
        op.f('ix_starred_file_file_id'), 'starred_file', ['file_id'], unique=False
    )
    op.create_index(
        op.f('ix_starred_file_user_id'), 'starred_file', ['user_id'], unique=False
    )
