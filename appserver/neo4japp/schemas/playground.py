...
class ContextPlaygroundRequestSchema(CamelCaseSchema):
    ...
    temperature = fields.Float(
        validate=marshmallow.validate.Range(min=0, max=2)
    )
    top_p = fields.Float()
    n = fields.Integer()
    stream = fields.Boolean()
    logprobs = fields.Integer(
        validate=marshmallow.validate.Range(min=0, max=5),
        allow_none=True
    )
    echo = fields.Boolean()
    stop = fields.List(
    ...
    presence_penalty = fields.Float(
        validate=marshmallow.validate.Range(min=-2, max=2)
    )
    ...
    logit_bias = fields.Dict(
        keys=fields.String(),
        values=fields.Float(
            validate=marshmallow.validate.Range(min=-100, max=100)
        )
    )
