    ...
    def _get_all_master_project_files(self) -> List[Files]:
        ...
        all_master_project_file_ids = [
            ...
            for result, in db.session.execute(
                get_descendants_of_file_query(master_initial_***ARANGO_USERNAME***_folder_id)
            )
        ]
        ...
        return db.session.query(
            ...
            Files.id.in_([master_initial_***ARANGO_USERNAME***_folder_id] + all_master_project_file_ids)
        ).order_by(
            asc(Files.path)
        ).all()
