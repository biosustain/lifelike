...
class GraphMixin(GraphConnection):
    ...
    def exec_read_query_with_params(self, query: str, values: dict):
        try:
            return self.graph.read_transaction(lambda tx: list(tx.run(query, **values)))
        except BrokenPipeError:
            raise BrokenPipeError(
                'The graph connection became stale while processing data, '
                'Please refresh the browser and try again.')
    ...
    def exec_write_query_with_params(self, query: str, values: dict):
        try:
            return self.graph.write_transaction(lambda tx: list(tx.run(query, **values)))
        except BrokenPipeError:
            raise BrokenPipeError(
                'The graph connection became stale while processing data, '
                'Please refresh the browser and try again.')
    ...
    def get_nodes_by_ids(self, entity_type):
