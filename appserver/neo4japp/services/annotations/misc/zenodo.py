...
def main():
    ...
    with app.app_context():
        ...
        sess = driver.session()
        print(sess.read_transaction(lambda tx: list(tx.run('MATCH (n:Taxonomy) RETURN n LIMIT 1'))))
        print(sess.read_transaction(lambda tx: list(tx.run('MATCH (n:Chemical) RETURN n LIMIT 1'))))
        sess.close()
