...
class SortedAnnotation:
    ...
    def get_annotations_per_file(
        self, files
    ) -> Tuple[FileAnnotationTable, AnnotationLookupTable]:
...
class SumLogCountSA(SortedAnnotation):
    ...
    def get_annotations(self, files):
        ...
        df = DataFrame(files_annotations, columns=["file_id", "key"])
        ...
        for file in files:
            ...
            for annotation in annotations:
                ...
                if key in distinct_annotations:
                    ...
                    distinct_annotations[key] = {"annotation": annotation, "value": 1}
...
class MannWhitneyUSA(SortedAnnotation):
    ...
    def get_annotations(self, files):
        ...
        ds = (
            DataFrame(files_annotations, columns=["file_id", "key"])
            .groupby(["file_id", "key"])
            .size()
        )
        ...
        idx = pd.MultiIndex.from_product(
            [unique_file_ids, unique_keys], names=["file_id", "key"]
        )
        ...
        for key in unique_keys:
            ...
            distinct_annotations[key] = {
                "annotation": key_map[key],
                "value": -np.log(
                    mannwhitneyu(ds[mask], ds[~mask], alternative="greater").pvalue
                ),
            }
    ...
    def get_annotations(self, files):
        ...
        for file in files:
            ...
            filtered_annotations = filter(
                lambda d: d["enrichmentDomain"] != "Imported", annotations
            )
...
class MannWhitneyPerRowUSA(SortedAnnotation):
    ...
    def get_annotations(self, files):
        ...
        for file in files:
            ...
            df = (
                df.groupby(["id", "enrichmentGene", "enrichmentDomain"])
                .aggregate(dict(uuid="count", **take_first_annotation))
                .reset_index()
            )
            df = (
                df.groupby(["id", "enrichmentGene"])
                .aggregate(dict(uuid="max", **take_first_annotation))
                .rename(columns=dict(uuid=0))
                .reset_index()
            )
            ...
            for key, annotation in gen:
                ...
                distinct_annotations[key] = {
                    ...
                    "value": -np.log(
                        mannwhitneyu(
                            values[mask], values[~mask], alternative="greater"
                        ).pvalue
                    ),
                }
    ...
    def get_annotations(self, files):
        ...
        for file in files:
            ...
            for annotation in filtered_annotations:
                ...
                distinct_annotation = distinct_annotations.get(
                    annotation_id, {"annotation": annotation, "value": set()}
                )
