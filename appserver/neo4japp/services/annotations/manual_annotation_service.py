...
from .utils.graph_queries import (
    ...
    get_create_lifelike_global_inclusion_query,
        ...
        if inclusion_type == ManualAnnotationType.INCLUSION.value:
            ...
            if check['node_exist'] and (
                ...
                query_fn, params = queries.get(
                    entity_type,
                    (get_create_lifelike_global_inclusion_query, all_params),
                )
                ...
                try:
                    query = get_create_lifelike_global_inclusion_query()
                    execute_arango_query(
                        db=get_db(self.arango_client),
                        query=query,
                        **all_params,
    ...
    def _global_annotation_exists_in_kg(self, values: dict):
        ...
        if check['node_exist']:
            ...
            try:
                check = execute_arango_query(
                    db=get_db(self.arango_client),
                    query=get_lifelike_global_inclusion_exist_query(),
                    **lifelike_params,
                )[0]
            except BrokenPipeError:
                raise
