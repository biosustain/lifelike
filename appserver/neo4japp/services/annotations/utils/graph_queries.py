...
collection_labels = {
    ...
    EntityType.ENTITY.value: 'lifelike',
    EntityType.COMPANY.value: 'lifelike',
    EntityType.LAB_SAMPLE.value: 'lifelike',
    EntityType.LAB_STRAIN.value: 'lifelike',
}
...
def get_lifelike_global_inclusions_by_type_query():
    return """
    FOR doc IN synonym
        FILTER 'GlobalInclusion' IN doc.labels
        FOR v, e IN 1..1 INBOUND doc has_synonym OPTIONS { vertexCollections: 'lifelike' }
            FILTER e.label == 'has_synonym'
...
def get_lifelike_global_inclusion_exist_query():
    return f"""
    LET res = (
        FOR n IN lifelike
...
def get_create_lifelike_global_inclusion_query():
