...
class StarredProjectListView(ProjectBaseView):
    def get(self):
        ...
        starred_projects, total = self.get_nondeleted_projects(
            filter=Projects.root_id.in_([file_id for file_id, in starred_file_ids])
        )
...
class ProjectListView(ProjectBaseView):
    ...
    def delete(self, targets):
        ...
        project_root_and_descendants = FilesystemBaseView.get_nondeleted_recycled_descendants(
            self,
            and_(
        ...
        if len(project_root_and_descendants) > 1:
            if reqursive:
        ...
        for file in project_root_and_descendants:
            file.delete()
