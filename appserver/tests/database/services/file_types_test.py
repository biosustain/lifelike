...
@pytest.mark.parametrize(
    'pair',
    [
        ('vnd.***ARANGO_DB_NAME***.filesystem/directory', True),
        ('vnd.***ARANGO_DB_NAME***.document/map', True),
        ('vnd.***ARANGO_DB_NAME***.document/enrichment-table', True),
...
@pytest.mark.parametrize(
    'pair',
    [
        ('vnd.***ARANGO_DB_NAME***.filesystem/directory', False),
        ('vnd.***ARANGO_DB_NAME***.document/map', False),
        ('vnd.***ARANGO_DB_NAME***.document/enrichment-table', True),
        ('application/pdf', True),
        ('image/png', False),
        ('text/plain', True),
...
def test_file_type_should_highlight_content_text_matches(pair):
    ...
    with app.app_context():
        ...
        assert (
            pair[1]
            == service.get(file.mime_type).should_highlight_content_text_matches()
        )
...
@pytest.mark.parametrize(
    'pair',
    [
        ('vnd.***ARANGO_DB_NAME***.filesystem/directory', BytesIO(), BytesIO()),
