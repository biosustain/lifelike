...
def test_read_url():
    with monkey_patch_controlled_conn():
        ...
        assert 'yo 🤙' == read_url(
            'http://example.com', max_length=1024 * 8
...
def test_read_url_has_custom_user_agent():
    with monkey_patch_controlled_conn():
        ...
        read_url(
            'http://example.com', headers={'User-Agent': 'some test'}, max_length=1000
        )
...
def test_read_url_enforces_read_length():
    with monkey_patch_controlled_conn():
        httpretty.register_uri(
            httpretty.GET, 'http://example.com', body=('hello' * 1000)
        )
...
def test_read_url_ignores_malicious_content_length_header():
    ...
    with monkey_patch_controlled_conn():
        httpretty.register_uri(
            httpretty.GET, 'http://example.com', body=generate_response
        )
...
def test_read_url_redirect_support(status_code):
    with monkey_patch_controlled_conn():
        httpretty.register_uri(
            ...
            forcing_headers={'Location': 'http://example.com'},
            status=status_code,
        )
        httpretty.register_uri(httpretty.GET, 'http://example.com', body='yo 🤙')
        assert 'yo 🤙' == read_url(
            'http://example.com/redirect', max_length=1024 * 8
        ).getvalue().decode('utf-8')
...
def test_read_url_has_redirect_limit():
    with monkey_patch_controlled_conn():
        httpretty.register_uri(
            httpretty.GET,
            'http://example.com/redirect',
            forcing_headers={'Location': 'http://example.com/redirect'},
            status=302,
        )
...
@pytest.mark.parametrize(
    'req_headers, resp_headers, expected',
    [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {'Accept': 'image/svg+xml'},
            {'Content-Type': 'image/svggggggxml'},
            'image/svg+xml',
        ),
