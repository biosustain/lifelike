...
import { BehaviorSubject, defer, EMPTY, from, iif, Observable, of, ReplaySubject, Subject } from 'rxjs';
import {
    catchError, filter, finalize, map, mergeMap, scan, shareReplay, startWith, switchMap, takeUntil, tap, withLatestFrom,
} from 'rxjs/operators';
...
export class PlaygroundComponent implements OnDestroy, OnChanges {
    constructor(private readonly explainService: ExplainService,
        ...
        this.promptComposer$.pipe(switchMap(({ componentRef }) => componentRef.instance.prompt$)).subscribe((prompt) => {
            this.form.controls.prompt.setValue(prompt);
        });
    }

    @ViewChild('promptForm', { static: true, read: ViewContainerRef }) promptFormView: ViewContainerRef;
    ...
    private readonly promptComposers: Map<PromptComposer, DynamicComponentRef<PromptComposer>> = new Map();
    ...
    readonly request$ = this.requestParams$.pipe(map((params) => {
        const loading$ = new BehaviorSubject(true);
        const error$ = new ReplaySubject<HttpErrorResponse>(1);
        const result$: Observable<any> = this.explainService.playground(params)
                                             .pipe(tap(() => loading$.next(false)), catchError((error) => {
                                                 error$.next(error);
                                                 return EMPTY;
                                             }), finalize(() => {
                                                 loading$.complete();
        ...
        return {
            params, loading$, error$, result$,
        };
    }), shareReplay({ bufferSize: 1, refCount: true }));

    readonly result$ = this.request$.pipe(switchMap(({ result$, params }) => iif(() => params.stream,
        result$.pipe(tap((result) => console.log(result)),
            filter(({ partialText }) => Boolean(partialText)),
            mergeMap(({ partialText }) => from(partialText.split('\n').filter(_identity))
                .pipe(map((partialTextLine: string) => JSON.parse(partialTextLine)),
                    scan((acc, partial) => _mergeWith((a, b, key, obj) => key === 'choices' ? _values(_mergeWith((ca, cb, ckey) => ckey
                                                                                                                                   === 'text'
                                                                                                                                   ? `${ ca
                                                                                                                                         ?? '' }${ cb
                                                                                                                                                   ?? '' }`
                                                                                                                                   : undefined)(_keyBy('index')(a),
                        _keyBy('index')(b),
                    )) : undefined)(acc, partial)),
                )),
    ...
    readonly requestCost$ = this.result$.pipe(withLatestFrom(this.requestParams$),
        map(([ { result }, params ]) => ChatGPT.getModelTokenCost(params.model) * result?.usage?.total_tokens),
    );
    ...
    readonly form = new FormGroup({
        ...
        model: new FormControl('text-davinci-003', [ Validators.required ], [
            (control: AbstractControl) => this.models$.pipe(map((models) => models.includes(control.value)
                                                                            ? null
                                                                            : { notAvailable: control.value })),
        ]),
        ...
        n: new FormControl(1, [
            CustomValidators.isInteger, ({ value }: FormControl) => {
        ...
        logitBias: new FormGroupWithFactory(() => new FormControl(0, [ Validators.min(-100), Validators.max(100) ]), {}),
    });
    ...
    readonly estimatedCost$ = defer(() => this.form.valueChanges.pipe(startWith(this.form.value), map(ChatGPT.estimateCost)));

    readonly groupedModels$: Observable<Record<string, string[]>> = this.models$.pipe(map(_flow(_groupBy(ChatGPT.getModelGroup),
        _mapValues(_sortBy(_identity)),
    )), shareReplay({ bufferSize: 1, refCount: true }));
    readonly cached$ = this.result$.pipe(map(({ cached }) => cached));

    readonly resultJSON$: Observable<any> = this.result$.pipe(map((result) => JSON.stringify(result.result ?? result, null, 2)),
        catchError((error) => of(`Error: ${ error }`)),
        takeUntil(this.destroy$),
    );

    readonly resultChoices$ = this.result$.pipe(map(({ result }) => result?.choices ?? []), catchError((error) => of(`Error: ${ error }`)));
        ...
        if (promptForm) {
            ...
            if (next?.componentRef) {
                ...
                const promptComposer = {
                    componentRef: this.dynamicViewService.createComponent<PromptComposer>(this.promptFormView,
                        promptForm.currentValue,
                        this.promptFormParams$,
                    ),
                };
    ...
    private parseFormValueToParams = (formValue): CompletitionsParams => _omit([
        _isEmpty(formValue.stop) ? 'stop' : null, _isEmpty(formValue.logitBias) ? 'logitBias' : null, formValue.n === 1 ? 'n' : null,
