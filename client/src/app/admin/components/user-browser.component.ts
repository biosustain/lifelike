...
export class UserBrowserComponent implements OnInit, OnDestroy {
  ...
  shownUsers: AppUser[] = [];
  filterQuery = "";
  loadTask: BackgroundTask<void, ResultList<PrivateAppUser>> =
    new BackgroundTask(() => this.accountService.getUsers());
        ...
        this.accountService
          ...
          .subscribe(
            (user: AppUser) => {
              ...
              this.refresh();
              this.snackBar.open(`User ${user.username} created!`, "close", {
                duration: 5000,
              });
            },
            () => {
  ...
  displayUpdateDialog() {
    for (const selectedUser of this.shownUsers.slice().reverse()) {
      if (this.selection.isSelected(selectedUser)) {
        ...
        modalRef.componentInstance.isSelf =
          this.currentUser.hashId === selectedUser.hashId;
              ...
              .subscribe(
                () => {
                  ...
                  if (this.currentUser.hashId === selectedUser.hashId) {
                    this.store.dispatch(
                      AuthActions.updateUserSuccess({ userUpdateData })
                    );
                    this.currentUser = {
                      ...this.currentUser,
                      ...userUpdateData,
