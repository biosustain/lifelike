...
export class AnnotationTableComponent implements OnInit, OnDestroy {
  ...
  readonly loadTask: BackgroundTask<
    PaginatedRequestOptions,
    ResultList<GlobalAnnotationListItem>
  > = new BackgroundTask((locator: PaginatedRequestOptions) =>
    this.globalAnnotationService.getAnnotations(
      locator,
      this.globalAnnotationType
    )
  );
  ...
  ngOnInit() {
    this.subscriptions.add(
      this.loadTask.results$.subscribe(({ result: annotations }) => {
  ...
  goToPage(page: number) {
    ...
    this.subscriptions.add(
      this.globalAnnotationService
        .getAnnotations(this.locator, this.globalAnnotationType)
  ...
  deleteAnnotation(objects: readonly GlobalAnnotationListItem[]) {
    const pids = objects.map((r: GlobalAnnotationListItem) => [
      r.globalId,
      r.synonymId ? r.synonymId : -1,
    ]);
    this.subscriptions.add(
      this.globalAnnotationService.deleteAnnotations(pids).pipe().subscribe()
    );
    this.refresh();
  }
  ...
  exportGlobalExclusions() {
    ...
    this.subscriptions.add(
      this.globalAnnotationService
        .exportGlobalExclusions()
        .pipe(this.errorHandler.create({ label: "Export global exclusion" }))
        .subscribe((event) => {
          if (event.type === HttpEventType.DownloadProgress) {
            progressObservables[0].next(getProgressStatus(event, "...", "..."));
          } else if (event.type === HttpEventType.Response) {
            progressDialogRef.close();
            const filename = event.headers
  ...
  exportGlobalInclusions() {
    ...
    this.subscriptions.add(
      this.globalAnnotationService
        .exportGlobalInclusions()
        .pipe(this.errorHandler.create({ label: "Export global inclusion" }))
        .subscribe((event) => {
          if (event.type === HttpEventType.DownloadProgress) {
            progressObservables[0].next(getProgressStatus(event, "...", "..."));
          } else if (event.type === HttpEventType.Response) {
