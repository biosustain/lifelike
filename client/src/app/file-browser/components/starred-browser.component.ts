...
export class StarredBrowserComponent implements OnInit, OnDestroy {
  readonly loadTask: BackgroundTask<void, FilesystemObjectList> =
    new BackgroundTask(() => this.filesystemService.getStarred());

  searchText: string;
  filter$: BehaviorSubject<(item: FilesystemObject) => boolean> =
    new BehaviorSubject((item: FilesystemObject) => !isNil(item.starred));
  ...
  applyFilter(filter: string) {
    ...
    this.filter$.next(
      (item: FilesystemObject) =>
        !isNil(item.starred) &&
        FilesystemObject.normalizeFilename(item.effectiveName).includes(
          normalizedFilter
        )
    );
  }
