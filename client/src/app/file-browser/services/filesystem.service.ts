...
export class FilesystemService {
  ...
  protected lmdbsDates$ = this.http.get<object>(`/api/files/lmdbs_dates`);

  search(options: ObjectSearchRequest): Observable<FilesystemObjectList> {
    return this.http.post<ResultList<FilesystemObjectData>>(`/api/filesystem/search`, options).pipe(
      map((data) => {
  ...
  bulkCreate(data: FormData, transactionId: string): ConnectableObservable<HttpEvent<string>> {
    return this.http.post(`/api/filesystem/objects/bulk-upload`, data, {
      observe: 'events',
      reportProgress: true,
      // @ts-ignore
      responseType: 'text',
   ...
   */
  open(hashId: string): Observable<FilesystemObject> {
  ...
  get(hashId: string): Observable<FilesystemObject> {
    return this.http
      .get<SingleResult<FilesystemObjectData>>(
        `/api/filesystem/objects/${encodeURIComponent(hashId)}`
      )
      .pipe(map((data) => new FilesystemObject().update(data.result)));
  }
  ...
  getMapContent(hashId: string): Observable<Blob> {
    return this.http.get(`/api/filesystem/objects/${encodeURIComponent(hashId)}/map-content`, {
      responseType: 'blob',
    });
  }
  ...
  getAllEnrichmentTables() {
    return this.http
      .get<{ result: string[] }>(`/api/filesystem/enrichment-tables`, {
        responseType: 'json',
      })
  ...
  save(
    ...
    return this.http
      .patch<ResultMapping<FilesystemObjectData>>(
        `/api/filesystem/objects`,
        objectToMixedFormData({
          ...changes,
          hashIds,
        })
      )
  ...
  delete(
    ...
    return this.http
      .request<ResultMapping<FilesystemObjectData>>('DELETE', `/api/filesystem/objects`, {
      ...
      .pipe(
        tap((data) =>
          hashIds.forEach((hashId) =>
            this.recentFilesService.deleteFromList({ hashId } as FilesystemObjectData)
          )
        ),
        map((data) => {
          const ret: { [hashId: string]: FilesystemObject } = updateWithLatest || {};
  ...
  getBackupContent(hashId: string): Observable<Blob | null> {
    return this.http
      .get(`/api/filesystem/objects/${encodeURIComponent(hashId)}/backup/content`, {
        responseType: 'blob',
      })
      .pipe(
        catchError((e) => {
  ...
  deleteBackup(hashId: string): Observable<{}> {
    return this.http.delete<{}>(`/api/filesystem/objects/${encodeURIComponent(hashId)}/backup`, {});
  }

  getVersionHistory(
    fileHashId: string,
    options: PaginatedRequestOptions = {}
  ): Observable<ObjectVersionHistory> {
    return this.http
      .get<ObjectVersionHistoryResponse>(
  ...
  getVersionContent(hashId: string): Observable<Blob> {
    return this.http.get(`/api/filesystem/versions/${encodeURIComponent(hashId)}/content`, {
      responseType: 'blob',
    });
  }
  ...
  getAnnotationHistory(
    hashId: string,
    options: Partial<PaginatedRequestOptions> = {}
  ): Observable<FileAnnotationHistory> {
    return this.http
      .get<FileAnnotationHistoryResponse>(
        `/api/filesystem/objects/${encodeURIComponent(hashId)}/annotation-history`,
        {
          params: serializePaginatedParams(options, false),
        }
      )
      .pipe(map((data) => new FileAnnotationHistory().update(data)));
  }

  annotate(object: FilesystemObject) {
    return this.lmdbsDates$.pipe(
      switchMap((lmdbsDates) =>
        object.children.items$.pipe(
          tap((items) => {
  ...
  private generateTooltipContent(file: PdfFile): string {
    const outdated = Array.from(Object.entries(this.lmdbsDates$)).filter(
      ([, date]: [string, string]) => Date.parse(date) >= Date.parse(file.annotationsDate)
    );
    if (outdated.length === 0) {
      return '';
    }
    return outdated.reduce(
      (tooltip: string, [name, date]: [string, string]) =>
    ...
    return this.http
      ...
      .pipe(
        ...
        catchError((e) => {
          if (e instanceof HttpErrorResponse) {
            if (e.status === 409) {
              const otherLocks = e.error.results.map((itemData) =>
                new ObjectLock().update(itemData)
              );
  ...
  deleteLock(hashId: string): Observable<{}> {
    return this.http
      .delete<unknown>(`/api/filesystem/objects/${encodeURIComponent(hashId)}/locks?own=true`)
      .pipe(map(() => ({})));
  }

  getHierarchy(directoriesOnly: boolean = false): Observable<FileHierarchyResponse> {
    return this.http.get<FileHierarchyResponse>(`/api/filesystem/objects/hierarchy`, {
      params: {
  ...
  updateStarred(hashId: string, starred: boolean) {
    return this.http
      .patch<SingleResult<FilesystemObjectData>>(
