...
export class FilesystemService {
  ...
  search(options: ObjectSearchRequest): Observable<FilesystemObjectList> {
    return this.http
      .post<ResultList<FilesystemObjectData>>(`/api/filesystem/search`, options)
      .pipe(
        map((data) => {
          const list = new FilesystemObjectList();
          list.results.replace(
            data.results.map((itemData) =>
              new FilesystemObject().update(itemData)
            )
          );
          return list;
        })
  ...
  create(
    ...
    return {
      ...
      body$: progress$.pipe(
        ...
        map(
          (response) => (response as any).body as SingleResult<FilesystemObject>
        )
      ),
    };
  }
  ...
  save(
    ...
    return this.http
      ...
      .pipe(
        map((data) => {
          const ret: { [hashId: string]: FilesystemObject } =
            updateWithLatest || {};
  ...
  delete(
    ...
    return this.http
      .request<ResultMapping<FilesystemObjectData>>(
        "DELETE",
        `/api/filesystem/objects`,
        {
      ...
      .pipe(
        tap((data) =>
          hashIds.forEach((hashId) =>
            this.recentFilesService.deleteFromList({
              hashId,
            } as FilesystemObjectData)
          )
        ),
        map((data) => {
          const ret: { [hashId: string]: FilesystemObject } =
          ...
          tap((items) => {
            items.forEach((child: FilesystemObject) => {
              if (child.type === "file") {
                ...
                child.annotationsTooltipContent =
                  this.generateTooltipContent(file);
              }
            });
          })
        )
  ...
  private generateTooltipContent(file: PdfFile): string {
    const outdated = Array.from(Object.entries(this.lmdbsDates$)).filter(
      ([, date]: [string, string]) =>
        Date.parse(date) >= Date.parse(file.annotationsDate)
    );
  ...
  getLocks(hashId: string): Observable<ObjectLock[]> {
    return this.http
      ...
      .pipe(
        map((data) => {
          return data.results.map((itemData) =>
  ...
  ): Observable<FileHierarchyResponse> {
  ...
  getStarred() {
    return this.http
      .get<ResultList<FilesystemObjectData>>(`/api/filesystem/objects/starred`)
      .pipe(
        map((data) => {
          const list = new FilesystemObjectList();
          list.results.replace(
