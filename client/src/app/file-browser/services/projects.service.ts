...
export class ProjectsService {
  ...
  list(options?: PaginatedRequestOptions): Observable<ProjectList> {
    return this.update$.pipe(
      ...
      switchMap(() =>
        this.http
          .get<ResultList<ProjectData>>(`/api/projects/projects`, {
  ...
  search(options: ProjectSearchRequest): Observable<ProjectList> {
    return this.http.post<ResultList<ProjectData>>(`/api/projects/search`, options).pipe(
      map((data) => {
        const projectList = new ProjectList(
          data.results.map((itemData) => new ProjectImpl().update(itemData))
        );
  ...
  create(request: ProjectCreateRequest) {
    return this.http
      .post<SingleResult<ProjectData>>(`/api/projects/projects`, request)
      .pipe(map((data) => new ProjectImpl().update(data.result)));
  }

  get(hashId: string): Observable<ProjectImpl> {
    return this.http
      .get<SingleResult<ProjectData>>(`/api/projects/projects/${encode(hashId)}`)
      .pipe(map((data) => new ProjectImpl().update(data.result)));
  }

  save(
    hashIds: string[],
    changes: Partial<BulkProjectUpdateRequest>,
    updateWithLatest?: { [hashId: string]: ProjectImpl }
  ): Observable<{ [hashId: string]: ProjectImpl }> {
    return this.http
      .patch<ResultMapping<ProjectData>>(`/api/projects/projects`, {
        ...changes,
        hashIds,
      })
      .pipe(
  ...
  getCollaborators(
    ...
    return this.http
      .get<ResultList<CollaboratorData>>(`/api/projects/projects/${hashId}/collaborators`, {
        params: serializePaginatedParams(options, false),
      })
      .pipe(
        map((data) => {
          ...
          collaboratorsList.results.replace(
            data.results.map((itemData) => new Collaborator().update(itemData))
          );
  ...
  saveCollaborators(
    ...
    return this.http
      .post<ResultList<CollaboratorData>>(`/api/projects/projects/${hashId}/collaborators`, request)
      .pipe(
        map((data) => {
          const collaboratorsList = new ModelList<Collaborator>();
          collaboratorsList.collectionSize = data.results.length;
  ...
  delete(
    ...
    return this.http
      .request<ResultMapping<FilesystemObjectData>>('DELETE', `/api/projects/projects`, {
