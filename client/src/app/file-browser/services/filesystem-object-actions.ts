...
export class FilesystemObjectActions {
    ...
    dialogRef.componentInstance.dismiss = (error) => {
      if (error) {
        this.messageDialog.display({
          ...
          message: `No export formats are supported for ${getObjectLabel(
            target
          )}.`,
  ...
  openCloneDialog(target: FilesystemObject): Promise<FilesystemObject> {
    ...
    object.filename = object.filename.replace(
      /^(.+?)(\.[^.]+)?$/,
      "$1 (Copy)$2"
    );
  ...
  openMoveDialog(targets: FilesystemObject[]): Promise<{
    ...
    dialogRef.componentInstance.emptyDirectoryMessage =
    ...
    dialogRef.componentInstance.accept = (destinations: FilesystemObject[]) => {
      ...
      return this.filesystemService
        .save(
          ...
          Object.assign(
            {},
            ...targets.map((target) => ({ [target.hashId]: target }))
          )
        )
        .pipe(
          finalize(() => progressDialogRef.close()),
          this.errorHandler.createFormErrorHandler(
  ...
  openDeleteDialog(
    targets: FilesystemObject[]
  ): Promise<ObjectEditDialogValue> {
    ...
    dialogRef.componentInstance.accept = () => {
      const progressDialogRef = this.createProgressDialog(
        `Deleting ${getObjectLabel(targets)}...`
      );
      return this.filesystemService
  ...
  reannotate(
    targets: FilesystemObject[]
  ): Promise<ResultMapping<AnnotationGenerationResultData>> {
    const progressDialogRef = this.createProgressDialog(
      "Parsing and identifying annotations..."
    );
