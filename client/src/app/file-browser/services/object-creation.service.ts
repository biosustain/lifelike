...
interface PutTask {
  ...
  annotation: Task<
    ResultMapping<AnnotationGenerationResultData> | { error: ErrorResponse }
  > & {
...
interface CreateToAnnotateStep {
  annotationTask?: HttpObservableResponse<
...
export type CreateResultMapping = Map<
  ObjectCreateRequest,
  CreationAnnotationResult
>;
...
export class ObjectCreationService {
    ...
    return {
      ...
      progress$: progress$.pipe(
          ...
          switch (event.type) {
            ...
            case HttpEventType.Sent:
              return {
                ...
                status: `Sending upload headers for ${
            ...
            case HttpEventType.DownloadProgress:
              return {
                mode: ProgressMode.Determinate,
                status: `Downloading server response for ${
                  request.filename || "file"
                }...`,
                value: event.loaded / event.total,
              };
        ...
        catchError((error) =>
          of({
            mode: ProgressMode.Determinate,
            status: `Error occurred during upload/parsing of ${
              request.filename || "file"
            }!`,
            value: 0,
            errors: [error],
  ...
  private composeAnnotationTask(
    ...
    return {
      body$: annotationCall$.pipe(
        switchMap((annotationCall) => annotationCall?.body$ ?? of(null))
      ),
          ...
          (annotationCall) =>
            annotationCall?.progress$.pipe(
                ...
                switch (event.type) {
                  ...
                  case HttpEventType.Response:
                    ...
                    if (result.success) {
                      return {
                        mode: ProgressMode.Determinate,
                        status: `Done uploading and annotating ${
                          request.filename || "file"
                        }...`,
                        value: 1,
                      };
                  ...
                  default:
                    return {
                      mode: ProgressMode.Determinate,
                      status: `Unknown state of annotating ${
                        request.filename || "file"
                      }...`,
                      value: 1,
                    };
              ...
              catchError((error) =>
                of({
                  mode: ProgressMode.Determinate,
                  status: `Error occurred during annotating of ${
                    request.filename || "file"
                  }!`,
                  value: 0,
                  errors: [error],
  ...
  private parseAnnotationStatus(
    ...
    return assign(
      (annotationResult as ResultMapping<AnnotationGenerationResultData>)
        ?.mapping?.[hashId] ?? {},
      {
        missing:
          (
            annotationResult as ResultMapping<AnnotationGenerationResultData>
          )?.missing?.includes(hashId) ?? false,
      },
  ...
  executePutWithProgressDialog(
    ...
    const putTasks: PutTask[] = zip(requests, annotationOptions).map(
      ([request, options]) => {
    ...
    const progressDialogRef = this.progressDialog.display({
      title: `Creating '${
        requests.length > 1 ? "Files" : requests[0].filename
      }'`,
      ...
      concatMap((resultBatch) => {
        const [resultsToAnnotate, resultsNotAnnotatable] = partition(
          ...
          ({ creation: { result, warnings } }) =>
            ...
            !some(
              warnings,
              ({ type }) => type === "TextExtractionNotAllowedWarning"
            )
        );
        const uniqeAnnotationConfigs: PDFAnnotationGenerationRequest[] =
          unionBy(
        ...
        return from(entries(resultsToAnnotateGroupedByAnnotationConfig)).pipe(
          ...
          tap(({ annotationTask, creationTaskBatch }) =>
            forEach(creationTaskBatch, ({ annotation: { call$ } }) =>
              call$.next(annotationTask)
            )
          )
        );
      }),
  ...
  openCreateDialog(
    ...
    dialogRef.componentInstance.accept = (requests: ObjectCreateRequest[]) => {
      const annotationOptions: PDFAnnotationGenerationRequest[] = requests.map(
        (request) => ({
      ...
      return this.executePutWithProgressDialog(requests, annotationOptions)
        ...
        .then(
          (resultsMapping) =>
            resultsMapping.values().next().value.creation.result
        );
    };
