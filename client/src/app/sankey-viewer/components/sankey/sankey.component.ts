...
export class SankeyComponent implements AfterViewInit, OnDestroy {
  ...
  private updateDOM(words) {
    ...
    const links = d3.select(this.links.nativeElement)
      ...
      .attr('d', link => {
        const {value, source, target} = link;
        ...
        // tslint:disable-next-line:no-bitwise
        const sourceY1 = (sourceNormalizer.normalize(value) * sourceHeight) + sourceY0;
        // tslint:disable-next-line:no-bitwise
        const targetY1 = (targetNormalizer.normalize(value) * targetHeight) + targetY0;
        // tslint:disable-next-line:no-bitwise
