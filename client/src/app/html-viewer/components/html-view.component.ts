...
import { HtmlFile } from 'app/interfaces/html-files.interface';
...
export class HtmlViewComponent implements OnDestroy, ModuleAwareComponent {
  ...
  @Output() fileOpen: EventEmitter<HtmlFile> = new EventEmitter();
  ...
  openhtmlSub: Subscription;
  ready = false;
  object?: FilesystemObject;
  // Type information coming from interface htmlSource at:
  // https://github.com/DefinitelyTyped/DefinitelyTyped/blob/master/types/htmljs-dist/index.d.ts
  htmlData: Array<Document>;
  currentFileId: string;
  addAnnotationSub: Subscription;
  removedAnnotationIds: string[];
  removeAnnotationSub: Subscription;
  htmlFileLoaded = false;
  ...
  constructor(
    ...
    this.loadTask = new BackgroundTask(([hashId]) => {
      return combineLatest(
        ...
        this.filesystemService.getContent(hashId).pipe(
          mapBlobToBuffer(),
          map(data => new TextDecoder('utf-8').decode(data))
        )
      );
    });
    ...
    this.openhtmlSub = this.loadTask.results$.subscribe(({
  ...
  /**
   * Open html by file_id along with location to scroll to
   * @param hashId - represent the html to open
   */
  openhtml(hashId: string) {
    if (this.object != null && this.currentFileId === this.object.hashId) {
      return;
    }
    this.htmlFileLoaded = false;
    this.ready = false;
  ...
  scrollInhtml(loc: Location) {
    this.pendingScroll = loc;
    if (!this.htmlFileLoaded) {
      console.log('File in the html viewer is not loaded yet. So, I cant scroll');
      return;
    }
  ...
  loadCompleted(status) {
    this.htmlFileLoaded = status;
    if (this.pendingScroll) {
      this.scrollInhtml(this.pendingScroll);
    }
...
// HTML is designed to allow programs that process text and
