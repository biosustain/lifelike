...
export class AsyncTextHighlighter {
  ...
  protected readonly renderQueue = new Map<
    TextHighlight,
    (fragment: DocumentFragment) => any
  >();
...
class TextHighlight implements NodeTextRange {
  ...
  get rects() {
    ...
    const tolerance = 0.5;
    const joinedRects = [];
    let joinRect: { x: number; y: number; width: number; height: number } =
      null;
    Array.from(range.getClientRects())
      ...
      .forEach((rect: DOMRect) => {
        if (joinRect === null) {
          ...
          joinedRects.push(
            new DOMRect(joinRect.x, joinRect.y, joinRect.width, joinRect.height)
          );
          joinRect = {
            x: rect.x,
            y: rect.y,
  ...
  createHighlights(container: Element): Element[] {
    ...
    for (const rect of this.rects) {
      ...
      el.className = this.focusOnNextRender
