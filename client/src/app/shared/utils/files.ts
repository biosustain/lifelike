...
export const mapBufferToJson = <T>(
  ...
  map(
    ary(textDecoder ? partialRight(bufferToJson, textDecoder) : bufferToJson)
  );
 ...
 */
export function mapJsonToGraph(): OperatorFunction<
  KnowledgeMapGraph,
  KnowledgeMapGraph
> {
  return map((graph) => {
...
export const mapBufferToJsons = <T>(
  ...
  map(
    ary(textDecoder ? partialRight(bufferToJsons, textDecoder) : bufferToJsons)
  );
