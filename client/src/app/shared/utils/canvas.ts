...
export function* wrapText<Metrics extends Pick<TextMetrics, "width">>(
  ...
  for (const block of text.split(/\r?\n/g)) {
    ...
    for (const lineWrappedOnTextBreak of getWidthFittedLines(
      words,
      width,
      measureText
    )) {
      const lineWrappedOnTextBreakOverflow =
        lineWrappedOnTextBreak.metrics.width > width;
      ...
      if (lineWrappedOnTextBreakOverflow) {
        ...
        const sylabes = lineWrappedOnTextBreak.text.match(
          REGEX.BETWEEN_SYLABES
        );
        if (sylabes) {
          for (const wordWrappedOnSylabes of getWidthFittedLines(
            ...
            yield {
              ...
              text:
                wordWrappedOnSylabes.text +
                (wordWrappedOnSylabes.remainingTokens ? "-" : ""),
