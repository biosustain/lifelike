...
const exampleFtpURLs = [
  'ftp://user:password@host:80/path',
];
...
describe('url should be same after decoding and encoding again', () => {
  it('AppURL', () =>
    exampleURLs.forEach(url =>
      expect(new AppURL(url).toString()).toBe(url),
    ),
  );
  it('HttpURL', () =>
    exampleHttpURLs.forEach(url =>
      expect(new HttpURL(url).toString()).toBe(url),
    ),
  );
  it('MailtoURL', () =>
    exampleMailToURLs.forEach(url =>
      expect(new MailtoURL(url).toString()).toBe(url),
    ),
  );
  it('FtpURL', () =>
    exampleFtpURLs.forEach(url =>
      expect(new FtpURL(url).toString()).toBe(url),
    ),
  );
});
...
describe('AppURL should init right url class', () => {
  it('AppURL -> HttpURL', () =>
    exampleHttpURLs.forEach(url =>
      expect(new AppURL(url)).toBeInstanceOf(HttpURL),
    ),
  );
  it('AppURL -> MailtoURL', () =>
    exampleMailToURLs.forEach(url =>
      expect(new AppURL(url)).toBeInstanceOf(MailtoURL),
    ),
  );
  it('AppURL -> FtpURL', () =>
    exampleFtpURLs.forEach(url =>
      expect(new AppURL(url)).toBeInstanceOf(FtpURL),
    ),
  );
});
