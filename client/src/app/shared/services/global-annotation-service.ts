...
export class GlobalAnnotationService {
  ...
  getAnnotations(
    ...
    globalAnnotationType: string
  ): Observable<ResultList<GlobalAnnotationListItem>> {
    return this.http.get<ResultList<GlobalAnnotationListItem>>(`${this.baseUrl}/global-list`, {
  ...
  deleteAnnotations(pids: number[][]): Observable<string> {
    return this.http
      .request<{ result: string }>('DELETE', `${this.baseUrl}/global-list`, {
  ...
  exportGlobalExclusions(): Observable<HttpEvent<Blob>> {
    return this.http.get(`${this.baseUrl}/global-list/exclusions`, {
      responseType: 'blob',
      observe: 'events',
      reportProgress: true,
    });
  ...
  exportGlobalInclusions(): Observable<HttpEvent<Blob>> {
    return this.http.get(`${this.baseUrl}/global-list/inclusions`, {
      responseType: 'blob',
      observe: 'events',
      reportProgress: true,
    });
