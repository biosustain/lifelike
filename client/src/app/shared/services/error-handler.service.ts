...
export class ErrorHandler {
  ...
  createUserError(
    ...
    const defaultErrorResponse: ErrorResponse = {
      ...
      title: "We're sorry!",
      message:
        "Looks like something went wrong on our end! Please try again in a moment.",
      additionalMsgs: [],
      stacktrace: null,
          ...
          .then(
            (errorResponse: ErrorResponse) =>
              new UserError({
                ...
                message:
                  this.httpCodeMessages[error.status] ?? errorResponse.message,
              })
          )
      );
    } else if (error instanceof UserError) {
      if (error.cause != null) {
        return Promise.resolve(this.createUserError(error.cause)).then(
          ({ stacktrace }) => {
      ...
      ({ title, message, additionalMsgs, stacktrace, transactionId }) =>
        this.loggingService
          ...
          .pipe(
            catchError(() => {
              console.warn(
                "Error could not be logged due to connection issue",
                error
              );
              return EMPTY;
            })
  ...
  showError(
    ...
    return Promise.allSettled([
      ...
      Promise.resolve(this.createUserError(error)).then(
        ({ title, message, additionalMsgs, stacktrace, transactionId }) =>
        ...
        switchMap((errorResponse) => {
          if (errorResponse?.fields) {
            ...
            for (const apiFieldKey of Object.keys(errorResponse.fields)) {
              const formFieldKey =
                apiFieldToFormFieldMapping[apiFieldKey] || apiFieldKey;
