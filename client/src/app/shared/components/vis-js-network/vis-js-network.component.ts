...
import {
  ...
  Node,
  Options,
} from "vis-network/dist/vis-network";
...
import {
  networkSolvers,
  networkEdgeSmoothers,
} from "./vis-js-network.constants";
...
export class VisJsNetworkComponent implements AfterViewInit {
  @Input() set config(config: Options) {
    ...
    if (!isNil(config.physics)) {
      ...
      if (!isNil(config.physics[this.currentSolver])) {
        this.currentCentralGravity =
          config.physics[this.currentSolver].centralGravity;
      } else {
    ...
    if (
      !isNil(config.edges.smooth) &&
      typeof config.edges.smooth === "object"
    ) {
      this.currentSmooth =
        config.edges.smooth.type || networkEdgeSmoothers.DYNAMIC;
    }
  ...
  setupSolverMap() {
    ...
    for (const solver in networkSolvers) {
      ...
      if (networkSolvers[solver]) {
        this.solverMap.set(
          networkSolvers[solver],
          toTitleCase(solver.split("_").join(" "))
        );
      }
    }
  }
  ...
  searchQueryChanged() {
    ...
    if (this.searchResults.length > 0) {
      this.networkData.nodes.update(
        this.searchResults.map(({ id }) => this.unhighlightNode(id))
      );
    }
    ...
    if (this.searchQuery !== "") {
      ...
      this.currentSearchIndex = 0;
      this.networkData.nodes.update(
        this.searchResults.map(({ id }) => this.highlightNode(id))
      );
  ...
  setupEventBinds() {
    ...
    this.networkGraph.on("select", ({ nodes, edges }) =>
      selected.next({ nodes, edges })
    );
    this.networkGraph.on("deselectNode", ({ nodes, edges }) =>
      selected.next({ nodes, edges })
    );
    this.networkGraph.on("deselectEdge", ({ nodes, edges }) =>
      selected.next({ nodes, edges })
    );
