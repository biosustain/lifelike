...
export class NodeRelationshipComponent {
  ...
  edgeDragStart(event: DragEvent) {
    ...
    dataTransfer.setData('application/***ARANGO_DB_NAME***-relationship', JSON.stringify({
      node1: {
        hash: node1Hash,
        ...
        data: {
          sources: this.snippets.map(snippet => {
            return {
              ...
              url: `https://www.ncbi.nlm.nih.gov/research/pubtator/?view=docsum&query=${snippet.publication.data.pmid}`
            } as Source;
          })
        },
        label: this.edge,
