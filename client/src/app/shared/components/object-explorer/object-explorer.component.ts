...
import { isArray, isFunction, isObject, isString, size } from "lodash-es";
...
export class ObjectExplorerComponent {
  ...
  getChildren(node) {
    if (isObject(node) as any) {
      ...
      const filterPrivateProperties: (value: any) => boolean = isDevMode()
        ? () => true
        : ([label]) => label[0] !== "_";
      return (
        Object.entries(node)
  ...
  hasChild(_, node: TreeNode) {
    return typeof node === "object" && size(node.children ? node.children : Object.keys(node)) > 0;
  }
}
