...
export class SearchableDropdownMenuComponent<Id, Item> implements OnChanges {
  @HostBinding("@blockInitialRenderAnimation") blockInitialRenderAnimation =
    true;
  ...
  filteredItems$ = combineLatest([
    ...
    map(([items, searchFunction]) =>
      items.filter(([id, item]) =>
        searchFunction(this.optionTextAccessor(id, item))
      )
    ),
  ...
  order = (a: KeyValue<number, string>, b: KeyValue<number, string>): number =>
    0;
  @Input() optionTextAccessor: (id, item: Item) => string = (id, item: Item) =>
    String(item);
