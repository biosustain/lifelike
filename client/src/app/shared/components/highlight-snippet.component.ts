...
export class HighlightSnippetComponent implements OnChanges {
  ...
  ngOnChanges() {
    ...
    const entry1Colors = this.legend.get(this.entry1Type) ||
      this.legend.get(`Literature${this.entry1Type}`) || ["#000", "#000"];
    ...
    const entry1StyleString = `
        ...
        `;
    const entry1TextJoinedByUnderscore = this.entry1Text.replace(
      spaceRegex,
      "_"
    );

    const entry2Colors = this.legend.get(this.entry2Type) ||
      this.legend.get(`Literature${this.entry2Type}`) || ["#000", "#000"];
    ...
    const entry2StyleString = `
            ...
            vertical-align: middle;
        `;
    const entry2TextJoinedByUnderscore = this.entry2Text.replace(
      spaceRegex,
      "_"
    );
    ...
    styleMap[
      this.entry1Text
    ] = `<div style="${entry1StyleString}">${this.entry1Text}</div>`;
    styleMap[
      this.entry2Text
    ] = `<div style="${entry2StyleString}">${this.entry2Text}</div>`;
    styleMap[
      entry1TextJoinedByUnderscore
    ] = `<div style="${entry1StyleString}">${this.entry1Text}</div>`;
    styleMap[
      entry2TextJoinedByUnderscore
    ] = `<div style="${entry2StyleString}">${this.entry2Text}</div>`;

    this.highlightedSnippet = this.snippet.replace(
      new RegExp(
        `\\b(${this.entry1Text}|${this.entry2Text}|${entry1TextJoinedByUnderscore}|${entry2TextJoinedByUnderscore})\\b`,
        "g"
      ),
      (match) => {
    ...
    this.trustedHTML = this.domSanitizer.bypassSecurityTrustHtml(
