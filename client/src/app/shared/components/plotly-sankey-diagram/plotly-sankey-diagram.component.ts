...
export class PlotlySankeyDiagramComponent implements AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    Plotly.newPlot(container, [this.data], this.config, {
      staticPlot: false,
    }).then(() => {
  ...
  styleLinks(links, style: any) {
    Object.keys(style).forEach((property) =>
      links.style(property, style[property])
    );
  }

  styleLinksConnectedToNode(nodeId: number, style: any) {
    const links = selectAll("path.sankey-link").filter((d: any) => {
  ...
  setupEvents() {
    ...
    nodeBoxes.on("mouseover", (node: any) => {
      const incomingLinks = selectAll("path.sankey-link").filter(
        (link: any) => link.link.target.index === node.index
      );

      const outgoingLinks = selectAll("path.sankey-link").filter(
        (link: any) => link.link.source.index === node.index
      );
