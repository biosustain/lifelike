 ...
 * from bubbling up.
 */
export function ignore404Errors<T>(): UnaryFunction<
  Observable<T>,
  Observable<T>
> {
  return pipe(
...
export const mapIterable = <O, R>(itrable, mapping, mappedObjectConstructor?) =>
  new (mappedObjectConstructor ?? itrable.constructor)(
    Array.from(itrable, mapping)
  );
