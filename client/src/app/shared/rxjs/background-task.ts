...
export class BackgroundTask<T, R> {
  ...
  readonly reducer: (newData: T, existingData: T) => T = (
    newData,
    existingData
  ) => {
  ...
  private nextStatus() {
    ...
    this.status$.next({
      ...
      retryInProgressShown:
        this.error && this.state !== TaskState.RetryLimitExceeded,
    ...
    switch (this.state) {
      ...
      case TaskState.Idle:
        ...
        setTimeout(
          ...
          !this.started ? this.initialDelay : this.delay
        );
        this.started = true;
        this.nextStatus();
        break;
