...
export class XMLAnnotationComponent extends XMLTag implements OnChanges {
  ...
  update() {
    ...
    this.idLink =
      (ENTITY_TYPE_MAP[type] as EntityType)?.links.find(
        (link) => link.name === idType
      )?.url + this.annoId;
    ...
    this.searchLinks = SEARCH_LINKS.map(({ domain, url }) => ({
      url:
        links[domain.toLowerCase()] ||
        url.replace(/%s/, encodeURIComponent(allText)),
      label: domain.replace("_", " "),
    }));
    this.searchInternalLinks =
      this.highlightTextService.composeSearchInternalLinks(allText);
    this.backgroundColor = this.toAnnotationBackgroundColor(
      this.getAnnotationColor()
    );
  }
  ...
  dragStart(event: DragEvent) {
    const { meta } = this;
    const text =
      meta.type === "Link" ? "Link" : meta.allText ?? this.textContent;
