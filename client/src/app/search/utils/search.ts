...
export interface GraphQueryParameters
  extends Omit<
      ...
      "query" | "domains" | "entities" | "page" | "limit"
    >,
    SearchableRequestOptions {
  domains?: string;
  entities?: string;
...
export const getGraphQueryParams: (
  params: Partial<GraphSearchParameters>
) => GraphQueryParameters = ({ query, domains, entities, page, ...rest }) => ({
  q: query,
...
export const createGraphSearchParamsFromQuery: (
  params: GraphQueryParameters
) => GraphSearchParameters = ({ q, domains, entities, page, ...rest }) => ({
  query: q,
...
export type ContentSearchParameters = PaginatedRequestOptions &
...
export const getContentSearchQueryAdvancedParams = ({
  types = [],
  folders = [],
}) =>
  omitBy(
...
export const getContentSearchQueryParams = (
  { q = "", ...params },
...
const createContentSearchAdvancedParamsFromQuery = (
  { types, folders, phrase, wildcards }: ContentSearchQueryParameters,
  searchTypesMap: Map<string, SearchType>
) =>
  omitBy(
    {
      types: types
        ? getChoicesFromQuery({ types }, "types", searchTypesMap)
        : [],
...
) => ContentSearchParameters = (params, { defaultLimit, searchTypesMap }) => ({
