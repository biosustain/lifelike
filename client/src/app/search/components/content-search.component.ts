...
export class ContentSearchComponent
  extends PaginatedResultListComponent<
    ContentSearchParameters,
    RankedItem<FilesystemObject>
  >
  implements OnInit, OnDestroy, ModuleAwareComponent
{
  ...
  get emptyParams(): boolean {
    ...
    const qExists =
      this.params.hasOwnProperty("q") && this.params.q.length !== 0;
    const typesExists =
      this.params.hasOwnProperty("types") && this.params.types.length !== 0;
    const foldersExists =
      this.params.hasOwnProperty("folders") && this.params.folders.length !== 0;
  ...
  constructor(
    ...
    objectTypeService.all().subscribe((providers: ObjectTypeProvider[]) => {
      this.searchTypes = flatten(
        providers.map((provider) => provider.getSearchTypes())
      );
      this.searchTypesMap = new Map(
        Array.from(this.searchTypes.values()).map((value) => [
          value.shorthand,
          value,
        ])
      );
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(
      this.route.queryParams
        .pipe(
          mergeMap((params) =>
            this.deserializeParams(params as ContentSearchQueryParameters)
          )
        )
        .subscribe((params) => {
  ...
  deserializeParams(
    params: ContentSearchQueryParameters
  ): Observable<ContentSearchParameters> {
    return of(
  ...
  highlightClicked(object: FilesystemObject, highlight: string) {
    ...
    const text = parser.parseFromString(highlight, "application/xml")
  ...
  highlightDisplayLimitChanged(
    object: DirectoryObject,
    change: HighlightDisplayLimitChange
  ) {
  ...
  extractAdvancedParamsFromString(q: string) {
    ...
    const extractedTypes =
      typeMatches == null
        ? []
        : typeMatches.map((typeVal) => typeVal.split(":")[1]);
  ...
  openSynonymSearch() {
    ...
    modalRef.result
      ...
      .then((expressionsToAdd: string[]) => {
        this.queryString = (
          (isNil(this.queryString) ? "" : `${this.queryString} `) +
          expressionsToAdd.join(" ")
        ).trim();
      })
