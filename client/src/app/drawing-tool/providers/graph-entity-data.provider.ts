...
export const GRAPH_ENTITY_TOKEN = new DataTransferToken<GraphEntity[]>(
  ...
  implements DataTransferDataProvider<(GraphEntity | URIData)[]>
{
  ...
  extract(
  ...
  ): DataTransferData<(GraphEntity | URIData)[]>[] {
    ...
    if (relationshipData) {
      const { node1, node2, edge } = JSON.parse(
        relationshipData
      ) as UniversalGraphRelationship;
      results.push({
        token: GRAPH_ENTITY_TOKEN,
        data: [
          {
