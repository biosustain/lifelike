...
import {
  IMAGE_TOKEN,
  ImageTransferData,
} from "../providers/image-entity-data.provider";
import {
  GraphEntity,
  GraphEntityType,
  UniversalEntityData,
...
export class GraphActionsService {
  ...
  fromDataTransferItems(
    ...
    return this.extractImageNodeActions(items, hoverPosition).then(
      (imageNodeActions) => actions.concat(imageNodeActions)
    );
  }

  extractImageNodeActions(
    items: DataTransferData<any>[],
    { x, y }: Point
  ): Promise<GraphAction[]> {
    ...
    return combineLatest(
      ...imageItems.map(({ data }: { data: ImageTransferData }) => {
        ...
        return iif(
          ...
          switchMap((blob) =>
            this.mapImageProviderService.doInitialProcessing(
              image_id,
              new File([blob], image_id)
            )
          ),
          map((dimensions) => {
  ...
  extractGraphEntityActions(
    ...
    for (const { type, entity } of entities) {
      if (type === GraphEntityType.Node) {
        ...
        actions.push(
          new GroupCreation(`Create group`, group, true, isSingularEntity)
        );
      }
    }
