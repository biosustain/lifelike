...
{
  @ViewChild("infoPanelSidebar", { static: false })
  infoPanelSidebarElementRef: ElementRef;
  ...
  registerGraphBehaviors() {
    ...
    this.graphCanvas.behaviors.add(
      ...
      new PasteKeyboardShortcutBehavior(
        this.graphCanvas,
    ...
    this.graphCanvas.behaviors.add(
      "image-upload",
      new ImageUploadBehavior(
        this.graphCanvas,
        this.mapImageProviderService,
        this.snackBar
      ),
      -100
    );
    ...
    this.graphCanvas.behaviors.add(
      "moving",
      new MovableEntity(this.graphCanvas),
      -10
    ); // from below
    this.graphCanvas.behaviors.add(
      "resize-handles",
        ...
        switchMap((newImages: ImageBlob[]) =>
          this.filesystemService.putBackup({
            ...
            contentValue: new Blob(
              [JSON.stringify(this.graphCanvas.getExportableGraph())],
              {
                type: MimeTypes.Map,
              }
            ),
  ...
  canCreateGroupFromSelection() {
    ...
    if (selection) {
      return (
        selection.filter((entity) => entity.type === GraphEntityType.Node)
          .length > 1 &&
  ...
  canExtendsGroupFromSelection() {
    ...
    if (selection) {
      return (
        selection.filter((entity) => entity.type === GraphEntityType.Node)
          .length > 0 &&
        selection.filter((entity) => entity.type === GraphEntityType.Group)
          .length === 1
      );
    }
    return false;
  ...
  private focusSidebar() {
    ...
    setTimeout(() => {
      const initialFocusElement =
        this.infoPanelSidebarElementRef.nativeElement.querySelector(
  ...
  reload() {
    ...
    if (this.unsavedChanges$.value) {
      if (
        confirm(
          "You have unsaved changes. Are you sure that you want to reload?"
        )
      ) {
        doReload();
      }
  ...
  createGroup() {
    ...
    if (graphCanvas) {
      ...
      const members = selection
        ...
        .reduce(
          (r, { type, entity }) =>
            type === GraphEntityType.Node ? r.concat(entity) : r,
  ...
  addToGroup() {
    ...
    const group = selection
      .filter((entity) => entity.type === GraphEntityType.Group)
      .pop().entity as UniversalGraphGroup;

    const potentialMembers = selection.flatMap((entity) =>
      entity.type === GraphEntityType.Node
        ? [entity.entity as UniversalGraphNode]
        : []
    );
    // No duplicates
    const newMembers = potentialMembers.filter(
      (node) => !group.members.includes(node)
    );
    this.graphCanvas?.execute(
      new GroupExtension("Add new members to group", group, newMembers)
    );
  }
}
