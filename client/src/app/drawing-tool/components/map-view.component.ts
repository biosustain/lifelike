  ...
  implements OnDestroy, AfterViewInit, ModuleAwareComponent
{
  ...
  save() {
    ...
    const { newImageHashes, deletedImages } =
      this.graphCanvas.getImageChanges();
    const newImageBlobs = newImageHashes.map((hash) =>
      this.mapImageProviderService.getBlob(hash)
    );
    ...
    const hashesOfLinked: string[] = Array.from(
      this.graphCanvas.getHashesOfLinked()
    );
  ...
  goToReturnUrl() {
    return Promise.resolve(this.shouldConfirmUnload)
      .then((shouldConfirmUnload) =>
        shouldConfirmUnload
          ? confirm("Leave editor? Changes you made may not be saved.")
          : true
      )
      .then(
        (navigate) =>
