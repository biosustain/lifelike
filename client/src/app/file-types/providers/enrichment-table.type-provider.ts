...
export const ENRICHMENT_TABLE_MIMETYPE =
...
export class EnrichmentTableTypeProvider extends AbstractObjectTypeProvider {
  ...
  createPreviewComponent(
    ...
    return contentValue$.pipe(
      mergeMap((blob) =>
        new EnrichmentDocument(this.worksheetService).loadResult(
          blob,
          object.hashId
        )
      ),
        ...
        item: {
          ...
          create: (
            ...
            dialogRef.componentInstance.title =
              "New Enrichment Table Parameters";
            ...
            return dialogRef.result.then(
              (value: EnrichmentTableEditDialogValue) => {
                ...
                return document
                  ...
                  .pipe(
                    ...
                    map(
                      (resultMapping) =>
                        resultMapping.values().next().value.creation.result
                    ),
  ...
  openEditDialog(
    ...
    return this.filesystemService
      ...
      .pipe(
        mergeMap((blob: Blob) =>
          new EnrichmentDocument(this.worksheetViewerService).loadResult(
            blob,
            target.hashId
          )
        ),
    ...
    if (biocycLabels) {
      ...
      if (tableHeaderLine2) {
        if (biocycLabels.length > 1) {
          ...
          tableHeaderLine2.splice(BIOC_ID_COLUMN_INDEX, 0, {
            name: "",
            span: "1",
          });
        }
      ...
      document.result.genes.forEach((gene, index) =>
        table.tableCells[index].splice(
          ...
          ...biocycLabels.map((label) => {
            ...
            if (geneDomainResult) {
              const biocycId =
                /[\?&]id=([^&#]*)/.exec(geneDomainResult.link)?.[1] ?? "";
        ...
        export: () =>
          this.filesystemService.getContent(object.hashId).pipe(
            ...
            map((blob) => {
              return new File(
                [blob],
                object.filename + "_for_graph_analysis.csv"
              );
            })
          ),
      },
        ...
        export: () => {
          return this.filesystemService.getContent(object.hashId).pipe(
            map((blob) => {
              return new File(
                [blob],
                object.filename + ".llenrichmenttable.json"
              );
            })
          );
        },
