...
export class MapTypeProvider extends AbstractObjectTypeProvider {
      ...
      (async () => {
        ...
        await Promise.all(imageProms).then((imageBlobs: Blob[]) => {
          for (let i = 0; i < imageIds.length; i++) {
            this.mapImageProviderService.setMemoryImage(
              imageIds[i],
              URL.createObjectURL(imageBlobs[i])
            );
          }
        });
        return unzipped;
        ...
        item: {
          ...
          create: (options?: CreateActionOptions) => {
            ...
            zip.file(
              ...
              { date: new Date(Date.UTC(1980, 1, 1)) }
            );
  ...
  getSearchTypes(): SearchType[] {
    return [Object.freeze({ id: MimeTypes.Map, shorthand: "map", name: "Maps" })];
  }
