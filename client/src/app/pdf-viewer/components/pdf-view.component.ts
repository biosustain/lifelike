...
export class PdfViewComponent
  implements OnDestroy, OnInit, ModuleAwareComponent
{
  constructor(
  ...
  loadTask: BackgroundTask<
    [string, Location],
    [FilesystemObject, ArrayBuffer, Annotation[]]
  >;
  ...
  @ViewChild(PdfViewerLibComponent, { static: false })
  pdfViewerLib: PdfViewerLibComponent;
  ...
  private entityTypeVisibilityMapChange$: Subject<EntityTypeVisibilityMap> =
  ...
  entityTypeVisibilityMapContainsUnchecked$ =
    this.entityTypeVisibilityMap$.pipe(
      ...
      iif(
        ...
        annotationHighlight?.index$.pipe(
          map((highlightedAnnotationIndex) => ({
            type: "annotation",
            value:
              annotationHighlight.firstAnnotation?.meta.allText ||
              annotationHighlight.id,
            changeValue: (value) =>
              this.pdfAnnService.highlightAllAnnotations(value),
            color: this.getAnnotationBackground(
              annotationHighlight.firstAnnotation
            ),
  ...
  ngOnInit() {
    this.pdfAnnService.foundHighlightAnnotations$.subscribe(
      (foundHighlightAnnotations) => {
        ...
        this.updateEntityTypeVisibilityMap((etvm) =>
          mapIterable(etvm, ([id, state]) => [
            id,
            foundHighlightAnnotationsTypes.has(id) || state,
          ])
        );
      }
    );
  ...
  scrollInPdf(loc: Location) {
    ...
    if (!this.pdfFileLoaded) {
      console.log(
        "File in the pdf viewer is not loaded yet. So, I cant scroll"
      );
      return;
    }
  ...
  setAllEntityTypesVisibility(state: boolean) {
    return this.updateEntityTypeVisibilityMap((etvm) =>
      mapIterable(etvm, ([id]) => [id, state])
    );
  }

  updateEntityTypeVisibilityMap(
    callback: (
      currentMap: ReadonlyEntityTypeVisibilityMap
