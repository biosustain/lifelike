...
export class AnnotationToolbarComponent {
  ...
  wrappingPage(node: Node) {
    let wrappingPage = (node as Element);
    while (wrappingPage && !(wrappingPage instanceof Element)) {
      wrappingPage = (wrappingPage as Element).parentElement;
    }
    return wrappingPage.closest('.page');
  }
