...
import { BehaviorSubject, combineLatest, Observable, ReplaySubject, Subject, defer, of, iif } from 'rxjs';
import { distinctUntilChanged, first, map, shareReplay, switchMap, tap, filter } from 'rxjs/operators';
...
export class PDFAnnotationService {
  ...
  sortedEntityTypeEntries$: Observable<EntityTypeEntry[]> = this.annotations$.pipe(
    map(annotations =>
      ENTITY_TYPES
        .map(entityType => new EntityTypeEntry(entityType, annotations.filter(ann => ann.meta.type === entityType.id)))
        .sort((a, b) => {
  ...
  foundHighlightAnnotations$ = this.highlightAnnotationId$.pipe(
    switchMap(highlightAnnotationId =>
      iif(
      ...
      ) as Observable<null | { id: number, annotations: Annotation[], index$: BehaviorSubject<number> }>,
    ),
  ...
  annotationHighlightChange$: Observable<AnnotationHighlightResult|null> = this.foundHighlightAnnotations$.pipe(
    switchMap(foundHighlightAnnotations =>
      iif(
  ...
  openExclusionPanel(annExclusion) {
    ...
    return dialogRef.result.then(
      exclusionData => this.annotationExclusionAdded({...exclusionData, ...annExclusion}),
    );
  }
    ...
    return this.updateAnnotationCollection(annotations =>
      annotations.map(ann =>
        (
          ...
          merge({}, // provide obj copy to ensure that update propagates
            ann,
            {
              meta: {
                isExcluded: true,
  ...
  annotationCreated(annotation: Annotation) {
    ...
    return dialogRef.result.then((annotateAll: boolean) => {
      ...
      return this.annotationService.addCustomAnnotation(
        this.hashId,
        {
          annotation,
          annotateAll,
  ...
  goToAnnotationHighlight(indexChangeCallback: (index: number) => number) {
    return this.annotationHighlightChange$.pipe(
      first(),
  ...
  annotationRemoved(uuid) {
    ...
    return dialogRef.result.then((removeAll: boolean) =>
        this.annotationService.removeCustomAnnotation(
          this.hashId,
          uuid,
          {
            removeAll,
  ...
  annotationExclusionAdded(exclusionData: AddedAnnotationExclusion) {
    return this.annotationService.addAnnotationExclusion(
      this.hashId,
      {
