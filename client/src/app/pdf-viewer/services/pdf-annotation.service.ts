...
export class PDFAnnotationService {
   ...
   */
  annotationEntityTypeMap$: Observable<Map<string, Annotation[]>> =
    this.annotations$.pipe(
      map((annotations) => {
        ...
        for (const annotation of annotations) {
          ...
          if (!entityType) {
            throw new Error(
              `unknown entity type ${annotation.meta.type} not in ENTITY_TYPE_MAP`
            );
          }
  ...
  sortedEntityTypeEntries$: Observable<EntityTypeEntry[]> =
    this.annotations$.pipe(
      map((annotations) =>
        ENTITY_TYPES.map(
          (entityType) =>
    ...
    switchMap((foundHighlightAnnotations) =>
      iif(
        ...
        of(foundHighlightAnnotations).pipe(
          switchMap(({ annotations, index$ }) =>
            index$.pipe(map((index) => annotations[index]))
          )
        )
      )
    )
                ...
                ? this.snackBar.open(
                    `Highlighted ${found} instance${found === 1 ? "" : "s"}  ` +
                      (firstAnnotation != null
                        ? `of '${firstAnnotation.meta.allText}' `
                        : "") +
  ...
  updateAnnotationCollection(
    updateCallback: (annotations: Annotation[]) => Annotation[]
  ) {
    ...
    if (uuids) {
      this.updateAnnotationCollection((currentAnnotations) =>
        currentAnnotations.filter(
          (ann: Annotation) => !uuids.includes(ann.uuid)
        )
      );
    }
  }
    ...
    return this.updateAnnotationCollection((annotations) =>
      annotations.map((ann) =>
        ...
        this.termsMatch(
          exclusionData.text,
          ann.textInDocument,
          exclusionData.isCaseInsensitive
        )
          ? merge(
  ...
  annotationCreated(annotation: Annotation) {
    ...
    return dialogRef.result.then(
      (annotateAll: boolean) => {
        ...
        return this.annotationService
          ...
          .pipe(
            this.errorHandler.create({ label: "Custom annotation creation" })
          )
          .toPromise()
          .then(
            (annotations: Annotation[]) => {
              ...
              this.snackBar.open("Annotation has been added", "Close", {
                duration: 5000,
              });
            },
            (err) => {
    ...
    this.updateAnnotationCollection((annotations) =>
  ...
  removeCustomAnnotation(uuid: string, request: CustomAnnotationDeleteRequest) {
    return this.annotationService.removeCustomAnnotation(
      this.hashId,
      uuid,
      request
    );
  ...
  goToAnnotationHighlight(indexChangeCallback: (index: number) => number) {
    return this.annotationHighlightChange$
      .pipe(
        first(),
        filter(
          (annotationHighlightChange) => !isNil(annotationHighlightChange)
        ),
    ...
    return dialogRef.result.then(
      (removeAll: boolean) =>
        this.annotationService
          ...
          .pipe(
            this.errorHandler.create({ label: "Custom annotation removal" })
          )
          .toPromise()
          .then(
            (response) => {
              this.removedAnnotationIds = response;
              this.snackBar.open("Removal completed", "Close", {
                duration: 10000,
              });
            },
            (err) => {
              this.snackBar.open(`Error: removal failed`, "Close", {
                duration: 10000,
              });
            }
          ),
    ...
    return this.annotationService
      ...
      .pipe(
        this.errorHandler.create({
          label: "Custom annotation exclusion addition",
        })
      )
      .toPromise()
      .then(
        (response) => {
          ...
          this.snackBar.open(
            `${exclusionData.text}: annotation has been excluded`,
            "Close",
            { duration: 10000 }
          );
        },
        (err) => {
          this.snackBar.open(
            `${exclusionData.text}: failed to exclude annotation`,
            "Close",
            { duration: 10000 }
          );
        }
  ...
  annotationExclusionRemoved({ type, text }) {
    return this.annotationService
      ...
      .pipe(
        this.errorHandler.create({
          label: "Custom annotation exclusion removal",
        })
      )
      .toPromise()
      .then(
        (response) => {
          ...
          this.snackBar.open("Unmarked successfully", "Close", {
            duration: 5000,
          });
        },
        (err: HttpErrorResponse) => {
          const error = err.error as ErrorResponse;
          this.snackBar.open(`${error.title}: ${error.message}`, "Close", {
            duration: 10000,
          });
        }
      );
  ...
  removeAnnotationExclusion(request: AnnotationExclusionDeleteRequest) {
    return this.annotationService.removeAnnotationExclusion(
      this.hashId,
      request
    );
  }
