...
export class AccountService implements OnDestroy {
  ...
  getUsers(hashId?: string): Observable<ResultList<PrivateAppUser>> {
    if (hashId) {
      return this.http.get<ResultList<PrivateAppUser>>(
        `${this.accountApi}/${hashId}`
      );
    }
  ...
  getUserBySubject(subject: string): Observable<PrivateAppUser> {
    return this.http.get<PrivateAppUser>(
      `${this.accountApi}/subject/${subject}`
    );
  }
  ...
  changePassword(updateRequest: ChangePasswordRequest) {
    ...
    return this.http.post(`${this.accountApi}/${hashId}/change-password`, {
      newPassword,
      password,
    });
  }
