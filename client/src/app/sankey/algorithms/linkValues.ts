...
export const byProperty: (property: string) => ValueProcessingStep<TypeContext> = (property) =>
  // tslint:disable-next-line:only-arrow-functions // allowing non-arrow function so we can maintain execution context
  function (this: DefaultLayoutService, { links }) {
    links.forEach((l) => {
...
export const byArrayProperty: (property: string) => ValueProcessingStep<TypeContext> = (property) =>
  // tslint:disable-next-line:only-arrow-functions // allowing non-arrow function so we can maintain execution context
  function (this: DefaultLayoutService, { links }) {
    links.forEach((l) => {
