...
export abstract class StateControlAbstractService<
...
> {
  ...
  optionStateAccessor<MappingResult>(
    ...
    return this.optionStateMultiAccessor(
      ...
      mapping
        ? (options, state) =>
            mapping(options[optionProperty], state[stateProperty])
        : (options, state) =>
            options[optionProperty][state[stateProperty] as any]
  ...
  optionStateMultiAccessor<MappingResult>(
    ...
    mapping =
      mapping ??
      ((options, state) => ({ options, state } as any as MappingResult));
    return unifiedAccessor(this.options$, optionProperties).pipe(
      switchMap((options) =>
        unifiedAccessor(this.state$, stateProperties).pipe(
          ...
          debug(
            `optionStateMultiAccessor(${optionProperties}, ${stateProperties})`
          ),
          shareReplay(1)
        )
      )
