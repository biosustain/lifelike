import { AfterViewInit, ElementRef, NgZone, OnDestroy, OnInit, ViewChild } from "@angular/core";
...
import { animationFrameScheduler, combineLatest, Subject } from "rxjs";
import { assign, groupBy, isEmpty, partial } from "lodash-es";
...
import { SankeyId, SankeyLinkInterface, TypeContext } from "app/sankey/interfaces";
...
import { EntityType, Match } from "../interfaces/search";
  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  vbsubs = this.viewBox$.subscribe(({ width, height }) => {
    this.width = width;
    this.height = height;
  ...
  renderedLinks$ = combineLatest([this.sankey.graph$, this.sankey.linkPath$]).pipe(
    map(([{ links }, linkPath]) => {
      ...
      const layerWidth = ({ source, target }: SankeyLinkInterface) =>
        Math.abs(target.layer - source.layer);

      // save selection in this point so we can forward d3 lifecycle groups
      return this.linkSelection
        .data<Base["link"]>(
          links.sort((a, b) => layerWidth(b) - layerWidth(a)),
          id
        )
        .join(
          (enter) =>
        ...
        .call((join) => join.select("title").text(linkTitle));
    }),
    shareReplay(1)
  );
  renderedNodes$ = combineLatest([this.sankey.graph$, this.sankey.fontSize$]).pipe(
    map(([{ nodes }, fontSize]) => {
      ...
      return this.nodeSelection
        ...
        .join(
          (enter) =>
            enter
              ...
              .call((enterNode) => enterNode.append("title").text(nodeLabel)),
          ...
          (update) =>
            update
              .call((enterNode) => {
                ...
                updateNodeText(
                  enterNode.select("foreignObject").attr("text-anchor", "end")
                  // todo: reenable when performance improves
                  // .transition().duration(RELAYOUT_DURATION)
                );
              })
        ...
        .call((joined) => {
          updateNodeRect(
            joined.select("rect").style("fill", nodeColor as d3_ValueFn<any, Base["node"], string>)
          );
          joined.select("foreignObject").call((textGroup) => {
            textGroup.select("div").text(this.sankey.nodeLabel);
          });
        });
    }),
    shareReplay(1)
  ...
  updateSearch$ = this.search.preprocessedMatches$.pipe(
    ...
    publish((matches$) =>
      combineLatest([
        matches$.pipe(
          map(({ [EntityType.Node]: nodes = [] }) =>
            (nodes as Match[]).map<SankeyId>(({ id }) => id)
          ),
          switchMap((ids) =>
            this.renderedNodes$.pipe(
              tap((renderedNodes) => {
        ...
        matches$.pipe(
          map(({ [EntityType.Link]: links = [] }) =>
            (links as Match[]).map<SankeyId>(({ id }) => id)
          ),
          switchMap((ids) =>
            this.renderedLinks$.pipe(
              tap((renderedLinks) => {
                if (isEmpty(ids)) {
                  ...
                  renderedLinks
                    .attr("searched", ({ id, originLink }: any) =>
                      ids.includes(id ?? originLink?.id)
                    )
    ...
    this.lastHoveredNode$.pipe(
      ...
      switchMap(({ data, element }) => {
        ...
        return this.renderedLinks$.pipe(
          tap((linkSelection) => {
            linkSelection.filter((link) => links.includes(link)).raise();
  ...
  get linkSelection(): d3_Selection<any, Base["link"], any, any> {
    ...
    return d3_select(this.links.nativeElement).selectAll(function () {
  ...
  get nodeSelection(): d3_Selection<any, Base["node"], any, any> {
    // returns empty selection if DOM struct was not initialised
    return d3_select(this.nodes.nativeElement).selectAll(function () {
      // by default there is recursive match, not desired in here
      return this.children;
    });
  }
  ...
  initHover() {
    this.hoverUpdate$.pipe(takeUntil(this.destroyed$)).subscribe();
  }

  initSearch() {
    this.updateSearch$.pipe(takeUntil(this.destroyed$)).subscribe();
  }
  ...
  attachLinkEvents(d3Links) {
    d3Links.on("click", this.linkClick).on("mouseover", this.pathMouseOver);
  }
  ...
  linkClick(element, data) {
    return Promise.resolve(this.selection.toggleLink(data)).then(() =>
      Promise.resolve(this.clipboard.writeToClipboard(data.path)).then(
        () => this.snackBar.open(`Path copied to clipboard`, undefined, { duration: 500 }),
        console.error
      )
    );
  }
  ...
  dragmove(element, d) {
    ...
    d3_select(element).raise().attr("transform", `translate(${d.x0},${d.y0})`);
