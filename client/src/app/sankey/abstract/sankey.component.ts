...
export type DefaultSankeyAbstractComponent =
  SankeyAbstractComponent<TypeContext>;
  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  zoomAdjust = this.viewBox$
    ...
    .subscribe(({ widthDelta, heightDelta, widthChange, heightChange }) => {
      ...
      const k =
        Math.abs(widthChange - 1) > Math.abs(heightChange - 1)
          ? widthChange
          : heightChange;
  ...
  updateNodeText(texts) {
    ...
    return texts
      .attr(
        "transform",
        ({ x0, x1, y0, y1 }) => `translate(${(x1 - x0) / 2}, ${(y1 - y0) / 2})`
      )
  ...
  linkClick(element, data) {
    return Promise.resolve(this.selection.toggleLink(data)).then(() =>
      Promise.resolve(this.clipboard.writeToClipboard(data.path)).then(
        () =>
          this.snackBar.open(`Path copied to clipboard`, undefined, {
            duration: 500,
          }),
        console.error
      )
