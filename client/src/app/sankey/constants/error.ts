...
export const ErrorMessages = {
  ...
  missingEntityType: (type: string) => `Entity type ${type} is not supported`,
  missingEntity: (id: string | number) => `No entity found for id ${id}`,
  ...
  missingNodeValueAccessor: (id: string | number) => ErrorMessages.missingValueAccessor("node", id),
  missingLinkValueAccessor: (id: string | number) => ErrorMessages.missingValueAccessor("link", id),
  incorrectValueAccessor: (type: string, predefinedProperties: Iterable<string>) =>
    `Predefined ${type} value accessor accesses not existing properties: ${Array.from(
  ...
  noColorMapping: (label: string) => `There is no color mapping for label: ${label}`,
