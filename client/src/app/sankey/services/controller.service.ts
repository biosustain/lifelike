...
export function patchReducer(patch, callback) {
  return switchMap(
    (stateDelta) => callback(stateDelta, patch) ?? of(stateDelta)
  );
}
  ...
  implements OnDestroy
{
  constructor(
    ...
    this.delta$
      .pipe(
        ...
        switchMap((delta) =>
          from(this.moduleContext.appLink).pipe(
            map((href) => ({ ...delta, href }))
          )
        )
      )
      ...
      switchMap((delta) =>
        iif(
          ...
          this.data$.pipe(
            map(
              ({ graph: { traceNetworks } }) =>
                traceNetworks[delta.networkTraceIdx]
            ),
                  ...
                  () =>
                    this.messageDialog
                      .display({
                        ...
                        additionalMsgs: [
                          "View might have been deleted or renamed.",
                        ],
                      ...
                      .finally(() =>
                        this.patchState({ viewName: null }).toPromise()
                      ) // init corrected update
                ),
  ...
  options$ = this.data$.pipe(
    map((data) =>
      _mergeAll([{}, this.staticOptions, this.extractOptionsFromGraph(data)])
    ),
    debug("options$"),
    shareReplay<SankeyOptions>(1)
  );

  networkTrace$ = this.optionStateAccessor<TraceNetwork>(
    "networkTraces",
    "networkTraceIdx"
  );
  ...
  view$ = this.views$.pipe(
    switchMap((views) =>
      this.viewName$.pipe(map((viewName) => views[viewName as string] ?? null))
    ),
    debug("view"),
  ...
  oneToMany$ = this.networkTrace$.pipe(
    map(
      ({ sources, targets }) => Math.min(sources.length, targets.length) === 1
    )
  );
  private excludedProperties = new Set([
    "source",
    "target",
    "dbId",
    "id",
    ...
    map(({ nodes, getNodeById, links, graph: { traceNetworks } }) =>
      _transform(
        ...
        (pathReports, traceNetwork) =>
          (pathReports[traceNetwork.description] = traceNetwork.traces.map(
            (trace) => {
              const traceLinks = trace.edges.map((linkIdx) => ({
                ...links[linkIdx],
              }));
              const traceNodes =
                this.getNetworkTraceNodes(traceLinks).map(_clone);
              ...
              if (_isNil(source)) {
                this.warningController.warn(
                  ErrorMessages.missingNode(trace.source)
                );
              }
              if (_isNil(target)) {
                this.warningController.warn(
                  ErrorMessages.missingNode(trace.target)
                );
              }
  ...
  maximumLabelLength$ = unifiedSingularAccessor(
    this.options$,
    "maximumLabelLength"
  );
  maximumShortestPathPlusN$ = unifiedSingularAccessor(
    this.options$,
    "maximumShortestPathPlusN"
  );
  aligns$ = unifiedSingularAccessor(this.options$, "aligns");
  linkValueGenerators$ = unifiedSingularAccessor(
    this.options$,
    "linkValueGenerators"
  );
  linkValueAccessors$ = unifiedSingularAccessor(
    this.options$,
    "linkValueAccessors"
  );
  nodeValueGenerators$ = unifiedSingularAccessor(
    this.options$,
    "nodeValueGenerators"
  );
  nodeValueAccessors$ = unifiedSingularAccessor(
    this.options$,
    "nodeValueAccessors"
  );
  ...
  pickPartialAccessors = (obj) =>
    _pick(obj, ["nodeValueAccessorId", "linkValueAccessorId"]);

  getNetworkTraceBestFittingSizing$({ effectiveName }) {
    const persumedValueAccessorRegex = /reverse|rev.?pagerank/i.test(
      effectiveName
    )
      ? /rev.?pagerank/i
    ...
    return this.predefinedValueAccessors$.pipe(
      switchMap((predefinedValueAccessors) => {
        const persumedValueAccessorId = _keys(predefinedValueAccessors).find(
          (pva) => persumedValueAccessorRegex?.test(pva)
        );
        return iif(
          ...
          this.views$.pipe(
            ...
            map((view) =>
              _merge(
                ...
                previousDelta.viewName === delta.viewName
                  ? _omit(delta, "baseViewName")
                  : {}
              )
            )
          )
  ...
  private extractLinkValueProperties({
    links: sankeyLinks,
    graph: { sizing },
  }) {
  ...
  private extractNodeValueProperties({
    nodes: sankeyNodes,
    graph: { sizing },
  }) {
    const predefinedPropertiesToFind = new Set(
  ...
  private extractPredefinedValueProperties({
    sizing = {},
  }: {
