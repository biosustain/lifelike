  ...
  implements ServiceOnInit, OnDestroy
{
      ...
      switchMap((baseViewDelta) =>
        iif(
          ...
          this.common.delta$.pipe(
            map((commonDelta) =>
              omit(commonDelta, ["viewName", "networkTraceIdx"])
            ),
  ...
  linkPropertyAcessors: {
    ...
    [LINK_PROPERTY_GENERATORS.byArrayProperty]: (k) => ({
      ...
      postprocessing: ({ links }) => {
        links.forEach((l) => {
          l.multipleValues = l.multipleValues.map(
            (d) => d / (l.adjacentDivider || 1)
          ) as [number, number];
          // take max for layer calculation
        });
        return {
  ...
  onInit() {
    this.nodeValueAccessor$ = unifiedSingularAccessor(
      ...
      map((nodeValueAccessorId) =>
        nodeValueAccessorId
          ? this.nodeValueAccessors[
              nodeValueAccessorId as NODE_VALUE_GENERATOR
    ...
    this.linkValueAccessor$ = unifiedSingularAccessor(
      ...
      switchMap((linkValueAccessorId) =>
        iif(
          ...
          of(
            this.linkValueAccessors[linkValueAccessorId as LINK_VALUE_GENERATOR]
          ),
          unifiedSingularAccessor(
            ...
            map(
              (linkValueAccessors) =>
                this.linkPropertyAcessors[
                  linkValueAccessors[linkValueAccessorId]?.type
                ]?.(linkValueAccessorId) ??
                (this.warningController.warn(
