  ...
  return function (links) {
    links.forEach(({ trace }) => {
      this.warningController.assert(
        !_isNil(trace),
        ErrorMessages.missingLinkTrace
      );
    ...
    return _flow(
      ...
      _map((nodeLinks) => ({
        nodeLinks,
        avgGroup: medianBy(nodeLinks, ({ trace }) =>
          groups.indexOf(trace.group)
        ),
      })),
      _sortBy("avgGroup"),
...
export const DEFAULT_FONT_SIZE = 12 * 1.6;
...
type NodeColumns<Base extends TypeContext> = (
  | Base["node"]
  | LayerPlaceholder
)[][];
...
@Injectable()
export class LayoutService<Base extends TypeContext>
  extends SankeyAbstractLayoutService<Base>
  implements ServiceOnInit, OnDestroy
{
  ...
  fontSize$ = this.baseView.common.fontSizeScale$.pipe(
    map(
      (fontSizeScale) =>
        ...
        (d?, i?, n?) =>
          DEFAULT_FONT_SIZE * fontSizeScale
    )
  );
  ...
  assignValues(data) {
    return combineLatest([
      ...
      filter<[ValueGenerator<Base>, ValueGenerator<Base>]>((params) =>
        params.every((param) => !!param)
      ),
      tap(([nodeValueAccessor, linkValueAccessor]) => {
        const preprocessedLinks =
          linkValueAccessor.preprocessing.call(this, data) ?? {};
        const preprocessedNodes =
          nodeValueAccessor.preprocessing.call(this, data) ?? {};
        ...
        if (nodeValueAccessor.postprocessing) {
          Object.assign(
            data,
            nodeValueAccessor.postprocessing.call(this, data) ?? {}
          );
        }
        if (linkValueAccessor.postprocessing) {
          Object.assign(
            data,
            linkValueAccessor.postprocessing.call(this, data) ?? {}
          );
        }
        if (minValue < 0) {
          ...
          data.links.forEach((l) => {
            ...
            if (l.multipleValues) {
              l.multipleValues = l.multipleValues.map((v) => v - minValue) as [
                number,
                number
              ];
            }
  ...
  createVirtualNodes(data, columns) {
    ...
    const virtualPaths = new ExtendedMap<
      string,
      ExtendedArray<LayerPlaceholder>
    >();
    ...
    for (const link of data.links) {
      ...
      _range(virtualPathStartLayer, virtualPathEndLayer).forEach(
        (layer) =>
    ...
    return {
      nodesAndPlaceholders: _flatMap((column) => column)(
  ...
  layoutNodesWithinColumns(columns: Base["node"][][]) {
    return tap(({ ky, y0, y1, height }) =>
      columns.forEach((nodes) => {
        ...
        const spacerSize =
          freeSpace / (additionalSpacers ? length + 1 : length - 1);
  ...
  getVerticalLayoutParams$(
    ...
    columnsWithLinkPlaceholders: NodeColumns<Base>
  ) {
    return combineLatest([this.baseView.nodeHeight$, this.vertical$]).pipe(
      map(([nodeHeight, { height, y0, y1 }]) => {
        ...
        let ky = Math.max(
          ...
          min(
            columnsWithLinkPlaceholders,
            (c) => (height - (c.length - 1) * py) / sum(c, value)
          )
        );
        ...
        return {
          ...
          py: Math.min(
            dy,
            height /
              (max(columnsWithLinkPlaceholders, (c: Array<any>) => c.length) -
                1)
          ),
        } as VerticalContext;
      }),
  ...
  computeNodeLayers(data) {
    return this.baseView.common.align$.pipe(
      map((align) => {
        ...
        const columns: Base["node"][][] = new Array(x)
          .fill(undefined)
          .map(() => []);
        for (const node of nodes) {
          const i = Math.max(
            0,
            Math.min(x - 1, Math.floor(align.fn.call(null, node, x)))
          );
          node.layer = i;
          columns[i].push(node);
        }
  ...
  loadNodesPositionFromView(verticalContext, data, view: View) {
    return this.horizontal$.pipe(
      ...
      map((horizontalContext) => {
        ...
        // for faster lookup
        const entityById = new Map<string, SankeyNode>(
          data.nodes.map((d, i) => [String(d.id), d])
        );
  ...
  loadLinkOrderFromView(data, view: View) {
    return tap(() => {
      // for faster lookup
      const entityById = new Map<string, SankeyNode>(
        data.links.map((d, i) => [String(d.id), d])
      );
  ...
  onInit(): void {
    this.calculateLayout$ = this.baseView.networkTraceData$.pipe(
      ...
      switchMap((data: SankeyDocument) =>
        this.computeNodeLayers(data).pipe(
          switchMap(({ columns, x }) =>
            // Calculate the nodes' values, based on the values of the incoming and outgoing links
            this.assignValues(data).pipe(
              debug("assignValues"),
              switchMap(() => {
                const { nodesAndPlaceholders, columnsWithLinkPlaceholders } =
                  this.createVirtualNodes(data, columns);
                return this.getVerticalLayoutParams$(
                  nodesAndPlaceholders,
                  columnsWithLinkPlaceholders
                ).pipe(
                              ...
                              this.update.edited$.pipe(
                                ...
                                switchMap((edited) =>
                                  iif(
                                    ...
                                    this.loadNodesPositionFromView(
                                      verticalContext,
                                      data,
                                      view
                                    ).pipe(
  ...
  calculateLinkPathParams(link, normalize = true) {
    ...
    if (normalize) {
      ...
      const sourceNormalizer =
        sourceLinks.normalizer ??
        (sourceLinks.normalizer = normalizeGenerator(sourceValues));
      const targetNormalizer =
        targetLinks.normalizer ??
        (targetLinks.normalizer = normalizeGenerator(targetValues));
      ...
      if (multipleValues) {
        sourceY1 =
          sourceNormalizer.normalize(multipleValues[0]) * sourceHeight +
          sourceY0;
        targetY1 =
          targetNormalizer.normalize(multipleValues[1]) * targetHeight +
          targetY0;
      } else {
        sourceY1 =
          sourceNormalizer.normalize(linkValue) * sourceHeight + sourceY0;
        targetY1 =
          targetNormalizer.normalize(linkValue) * targetHeight + targetY0;
      }
    } else {
      let { width } = link;
      width = width || 1e-4;
