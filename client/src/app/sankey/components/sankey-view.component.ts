  ...
  implements OnInit, ModuleAwareComponent, AfterViewInit, ShouldConfirmUnload
{
  ...
  constructor(
    ...
    this.loadTask = new BackgroundTask((hashId) =>
      combineLatest([
        ...
        this.filesystemService
          .getContent(hashId)
          .pipe(mapBlobToBuffer(), mapBufferToJson()) as Observable<GraphFile>,
      ])
    );
          ...
          combineLatest([
            this.route.queryParams.pipe(
              map(({ return: returnUrl, ...params }) => {
                ...
                return this.parseUrlQueryParamsToState(
                  params as SankeyURLLoadParams
                );
              })
            ),
            this.route.fragment.pipe(
              switchMap((fragment) =>
                ...
                this.parseUrlFragmentToState(fragment).pipe(
                  ...
                  catchError((err, o) => {
                    this.snackBar.open(
                      "Referenced view could not be found.",
                      null,
                      { duration: 2000 }
                    );
    ...
    this.sankeyController.viewsUpdate$
      .pipe(switchMap(() => this.sankeyController.data$))
  ...
  predefinedValueAccessors$: Observable<any> =
  ...
  activeViewBaseName$: Observable<string> =
    this.viewController.activeViewBase$.pipe(
  ...
  baseViewContext$ = this.sankeyController.baseViewName$.pipe(
    scan((prev, baseViewName) => {
      ...
      const module =
        baseViewName === ViewBase.sankeyMultiLane
          ? MultiLaneBaseModule
      ...
      const injectComponent = (container, token) => {
        ...
        const factory =
          moduleRef.componentFactoryResolver.resolveComponentFactory(comp);
      ...
      const sankey = injectComponent(
        this.sankeySlot.viewContainerRef,
        SANKEY_GRAPH
      );

      this.dynamicComponentRef.set("sankey", sankey);
      this.dynamicComponentRef.set(
        "advanced",
        injectComponent(this.advancedSlot.viewContainerRef, SANKEY_ADVANCED)
      );
      this.dynamicComponentRef.set(
        "details",
        injectComponent(this.detailsSlot.viewContainerRef, SANKEY_DETAILS)
      );
  ...
  predefinedValueAccessor$ = this.baseView$.pipe(
    switchMap(
      (sankeyBaseViewControl) => sankeyBaseViewControl.predefinedValueAccessor$
    )
  );
  ...
  handleBeforeUnload(event) {
    return Promise.resolve(this.shouldConfirmUnload).then(
      (shouldConfirmUnload) => {
  ...
  get shouldConfirmUnload() {
    return this.pendingChanges$.pipe(first()).toPromise();
  }
  ...
  confirmDeleteView({ networkTraceIdx, viewName }): Promise<any> {
    return this.confirm({
      ...
      this.viewController.deleteView({ networkTraceIdx, viewName }).toPromise()
    );
  }
  ...
  updateOnlyViews(sankeyDocument: SankeyDocument) {
    ...
    const {
      graph: { trace_networks, ...fcGraph },
      ...fc
    } = this.fileContent;
    return {
      graph: {
        ...fcGraph,
        trace_networks: zip(traceNetworks, trace_networks).map(
          ([tn, fctn]) => ({
            ...fctn,
  ...
  saveFile(data: SankeyDocument) {
    ...
    const contentValue = new Blob([JSON.stringify(newContent)], {
      type: MimeTypes.Graph,
    });
    return this.object$
      .pipe(
        switchMap((object) =>
          this.filesystemService.save([object.hashId], { contentValue }).pipe(
            delay(1000),
            tap(() => {
  ...
  confirm({ header, body }): Promise<any> {
    const modal = this.modalService.open(SankeyConfirmComponent, {
              ...
              iif(
                ...
                defer(() =>
                  this.messageDialog.display({
                    ...
                    message:
                      "View name needs to be unique across all trace networks.",
  ...
  saveView(): Promise<any> {
    return this.viewController.activeViewName$
      .pipe(
        first(),
        switchMap((viewName) =>
          iif(
      ...
      .toPromise()
      .then(() => this.resetStretch());
  }

  saveViewAs(): Promise<any> {
    const dialogRef = this.modalService.open(SankeyViewCreateComponent, {
      ariaLabelledBy: "modal-basic-title",
    });
    dialogRef.componentInstance.accept = ({ viewName }) =>
      this.confirmCreateView(viewName).toPromise();
  ...
  zoomOut() {
    if (this.sankeySlot) {
      this.sankey.zoom.scaleBy(0.8, undefined, true);
    }
  }
  ...
  horizontalStretch() {
    if (this.sankeySlot) {
      this.sankey.horizontalStretch$.next(
        this.sankey.horizontalStretch$.value * 1.25
      );
    }
  }
  ...
  horizontalShrink() {
    if (this.sankeySlot) {
      this.sankey.horizontalStretch$.next(
        this.sankey.horizontalStretch$.value * 0.8
      );
    }
  }
  ...
  verticalStretch() {
    if (this.sankeySlot) {
      this.sankey.verticalStretch$.next(
        this.sankey.verticalStretch$.value * 1.25
      );
    }
  }
  ...
  verticalShrink() {
    if (this.sankeySlot) {
      this.sankey.verticalStretch$.next(
        this.sankey.verticalStretch$.value * 0.8
      );
    }
  }
  ...
  requestRefresh() {
    if (
      confirm(
        "There have been some changes. Would you like to refresh this open document?"
      )
    ) {
