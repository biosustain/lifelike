  ...
  implements OnInit, ModuleAwareComponent, AfterViewInit, ShouldConfirmUnload
{
            ...
            this.route.fragment.pipe(
              switchMap((fragment) =>
                ...
                this.parseUrlFragmentToState(fragment).pipe(
                  ...
                  catchError((err, o) => {
                    this.snackBar.open('Referenced view could not be found.', null, {
                      duration: 2000,
                    });
  ...
  readonly baseViewContext$ = this.sankeyController.baseViewName$.pipe(
    scan((prev, baseViewName) => {
      ...
      const module =
        baseViewName === ViewBase.sankeyMultiLane ? MultiLaneBaseModule : SingleLaneBaseModule;
  ...
  saveFile(data: SankeyDocument) {
    ...
    const contentValue = new Blob([JSON.stringify(newContent)], { type: MimeTypes.Graph });
    return this.object$
      .pipe(
        switchMap((object) =>
  ...
  selectNetworkTraceIdx(networkTraceIdx) {
    return this.baseViewContext$
      .pipe(
        ...
        switchMap(() => this.sankeyController.selectNetworkTrace(networkTraceIdx)),
        tap(() => {
          this.resetZoom(false);
          this.resetStretch();
        }),
  ...
  openBaseView(baseViewName: ViewBase): Promise<any> {
    return this.confirmUnloadWrapper(this.viewController.openBaseView(baseViewName))
      .pipe(
        take(1),
        catchError(() => of({}))
      )
  ...
  horizontalShrink() {
    if (this.sankeySlot) {
      this.sankey.horizontalStretch$.next(this.sankey.horizontalStretch$.value * 0.8);
    }
  }
  ...
  verticalShrink() {
    if (this.sankeySlot) {
      this.sankey.verticalStretch$.next(this.sankey.verticalStretch$.value * 0.8);
    }
  }
  ...
  openNewWindow() {
    return this.object$.pipe(map((object) => this.filesystemObjectActions.openNewWindow(object)));
  }
