  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  selectionUpdate$ = this.selection.selection$.pipe(
    map((selection) =>
      assign(
        mapValues(groupBy(selection, "type"), (group) =>
                  ...
                  this.renderedLinks$.pipe(
                    tap((renderedLinks) => {
                      if (isEmpty(traces)) {
                        ...
                        renderedLinks
                          .attr("transitively-selected", ({ trace }) =>
        ...
        switchMap((traceColorPaletteMap) =>
          combineLatest([this.renderedLinks$, this.renderedNodes$]).pipe(
            tap(([linksSelection, nodesSelection]) => {
              linksSelection
                .style("fill", (d: Base["link"]) =>
                  traceColorPaletteMap.get(d.trace.group)
                )
              ...
              nodesSelection
                ...
                .style(
                  "fill",
                  ({ sourceLinks, targetLinks, color }: Base["node"]) => {
                    ...
                    if (difference.size === 1) {
                      return traceColorPaletteMap.get(
                        difference.values().next().value.trace.group
                      );
                    } else {
                      return color;
                    }
