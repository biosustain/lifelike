...
export function inputCount(
  ...
  linkLayers.forEach((layer) => {
    ...
    for (const circularTrace of circularTraces) {
      const traceNormalLinks = normalLinks.filter(
        ({ trace }) => trace === circularTrace
      );
      const traceCircularLinks = circularLinks.filter(
        ({ trace }) => trace === circularTrace
      );
    ...
    circularLinks.forEach((circularLink) => {
      const circularLinkEstimation = traceCircularEstimation.get(
        circularLink.trace
      );
