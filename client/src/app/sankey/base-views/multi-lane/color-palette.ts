...
export const predefinedColorPaletteGenerator = (size, { palette = christianColors } = {}) => {
...
export const colorPaletteGenerator = (
  ...
  return (i: number) =>
    cubehelix(360 * hue(i, size), 2 * saturation(i, size), lightness(i, size), alpha(i, size));
};
...
export const expandingColorPalletGenerator = (size, {} = {}) => {
  ...
  const saturation = (i) =>
    skewNorm(0, 1, DEFAULT_SATURATION, saturationSteps, (i / hueSteps) % saturationSteps);
  const lightness = (i) =>
    skewNorm(
      0.2,
      0.8,
