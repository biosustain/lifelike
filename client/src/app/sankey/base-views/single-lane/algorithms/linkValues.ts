...
export function inputCount(this: SingleLaneLayoutService, data: Base["data"]) {
  const { sortedNodes, dt, maxExpectedValue } = initInputCountCalculation.call(
    this,
    data
  );
  calculateInputCountSkippingCircularLinksA.call(
    this,
    sortedNodes,
    dt,
    maxExpectedValue
  ...
  const linkLayers: Map<number, Base["link"][]> = getLinkLayers.call(
    this,
    data.links
  );
  const perLayerLinkEstimation = new ExtendedWeakMap<Base["link"], number[]>();
  linkLayers.forEach((layer) => {
    const [circularLinks, normalLinks] = partition(
      layer,
      ({ circular }) => circular
    );
    const circularTraces = new Set(
      flatMap(circularLinks, ({ traces }) => traces)
    );
    const traceCircularEstimation = new WeakMap<Base["trace"], number>();
    for (const circularTrace of circularTraces) {
      const traceNormalLinks = normalLinks.filter(({ traces }) =>
        traces.includes(circularTrace)
      );
      const traceCircularLinks = circularLinks.filter(({ traces }) =>
        traces.includes(circularTrace)
      );
      const traceNormalLinksValue = sumBy(
        traceNormalLinks,
        ({ value }) => value
      );
      ...
      const traceCircularLinkEstimation =
        Math.abs(traceNormalLinksValue - 1) / traceCircularLinks.length;
      traceCircularEstimation.set(circularTrace, traceCircularLinkEstimation);
    }
    circularLinks.forEach((circularLink) => {
      const circularLinkEstimation = sumBy(circularLink.traces, (trace) =>
        traceCircularEstimation.get(trace)
      );
    ...
    circularLinks.forEach((circularLink) => {
      ...
      this.warningController.assert(
        circularLink.value <= maxExpectedValue,
        "Input count algorithm fail - node value exceeds input node count"
      );
    });
  });
  // propagate changes
  calculateInputCountSkippingCircularLinksB.call(
    this,
    sortedNodes,
    dt,
    maxExpectedValue
