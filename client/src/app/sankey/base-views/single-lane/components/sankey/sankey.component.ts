...
import {
  SelectionEntity,
  SelectionType,
} from "app/sankey/interfaces/selection";
  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  focusedLink$ = this.search.searchFocus$.pipe(
    map(
      (searchFocus) => searchFocus?.type === EntityType.Link && searchFocus?.id
    ),
    debug("focusedLink$"),
    switchMap((id) =>
    ...
    return this.sankey.baseView.highlightCircular$.pipe(
      ...
      map((highlightCircular) => {
        ...
        for (const { direction, node, path } of objects2traverse) {
          const { graphRelativePosition, nextNode, nextLinks, traversedLinks } =
            helper[direction];
                          ...
                          map((colorLinkByType) => {
                            if (colorLinkByType) {
                              ...
                              renderedLinks.attr(
                                ...
                                ({ graphRelativePosition }) =>
                                  Boolean(graphRelativePosition)
                              );
                            } else {
                              renderedLinks.attr(
                                "transitively-selected",
                                ({ graphRelativePosition }) =>
                                  graphRelativePosition ?? false
                              );
                            }
                            return renderedLinks
                              .filter(({ graphRelativePosition }) =>
                                Boolean(graphRelativePosition)
                              )
                              .raise();
                          })
      ...
      .pipe(
        ...
        switchMap((colorLinkByType) =>
          this.renderedLinks$.pipe(
            tap((linksSelection) => {
              if (colorLinkByType) {
                linksSelection.attr("type", ({ label }) =>
                  label?.toLowerCase()
                );
                // .each(function({label}) {
  ...
  initFocus() {
    forkJoin(this.focusedLink$, this.focusedNode$)
