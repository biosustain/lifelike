...
@Component({
  selector: 'app-enrichment-word-cloud',
  templateUrl: './enrichment-word-cloud.component.html',
  styleUrls: ['./enrichment-word-cloud.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class EnrichmentWordCloudComponent implements AfterViewInit, OnDestroy {
  ...
  @Input('data') set data(data) {
    ...
    if (
      Array.isArray(data) &&
      data.every((d) => typeof d === 'string' && (count[d] = (count[d] || 0) + 1))
    ) {
  ...
  ngAfterViewInit() {
    ...
    this.resizeObserver = createResizeObserver(
      ...
      this.cloudWrapper.nativeElement
    );
  }
  ...
  onResize(width, height) {
    ...
    d3.select(this.svg.nativeElement).attr('width', width).attr('height', height);

    // Get and update the grouping element
    d3.select(this.g.nativeElement).attr(
      'transform',
