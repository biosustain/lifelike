  ...
  checkTermsOfService$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(({ credential }) => {
        ...
        const outOfDate = cookie
          ? new Date(TERMS_OF_SERVICE.updateTimestamp) > new Date(cookie)
          : false;
  ...
  agreeTermsOfService$ = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap(({ credential, timeStamp }) => {
        ...
        return from([AuthActions.login({ credential })]);
      })
    )
  );
      ...
      exhaustMap(() => {
        return from([
          SnackbarActions.displaySnackbar({
            payload: {
              message:
                "Access can not be granted until the password is changed",
              action: "Dismiss",
              config: { duration: 10000 },
            },
          }),
  ...
  updateOAuthUser$ = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap(({ userUpdateData }) => {
        ...
        return this.keycloakAccountService
          .updateCurrentUser(userUpdateData)
          .pipe(
            map(() => {
              progressDialogRef.close();
          ...
          catchError((err: HttpErrorResponse) => {
            return from([
              AuthActions.loginFailure({
                message: (err.error as ErrorResponse).message,
              }),
            ]);
          })
        );
  ...
  loginSuccess$ = createEffect(
    () =>
      this.actions$.pipe(
        ...
        withLatestFrom(
          this.store$.pipe(select(AuthSelectors.selectAuthRedirectUrl))
        ),
    ...
    this.actions$.pipe(
      ...
      exhaustMap(({ oauthLoginData }) => {
        return this.accountService
          .getUserBySubject(oauthLoginData.subject)
          .pipe(
            map((user) =>
              AuthActions.oauthLoginSuccess({
                ***ARANGO_DB_NAME***User: user,
                oauthUser: oauthLoginData,
              })
            ),
            catchError((err: HttpErrorResponse) => {
