...
export class VisualizationService {
  ...
  getBatch(query: string) {
    return this.http
      .get<{ result: Neo4jResults<any, any> }>(`${this.baseUrl}/batch`, {
        params: { data: query },
      })
  ...
  expandNode(nodeId: number, filterLabels: string[]) {
    return this.http
      .post<{ result: Neo4jResults<any, any> }>(`${this.baseUrl}/expand`, {
        nodeId,
        filterLabels,
      })
      .pipe(map((resp) => resp.result));
  ...
  getReferenceTableData(request: ReferenceTableDataRequest) {
    return this.http
      .post<{ result: GetReferenceTableDataResult }>(
        `${this.baseUrl}/get-reference-table-data`,
        {
          nodeEdgePairs: [...request.nodeEdgePairs].sort(
            (
              {
                node: { displayName: a1 },
                edge: { originalFrom: a2, originalTo: a3 },
              },
              {
                node: { displayName: b1 },
                edge: { originalFrom: b2, originalTo: b3 },
              }
            ) => {
              const a = `${a1}${a2}${a3}`;
              const b = `${b1}${b2}${b3}`;
  ...
  getSnippetsForEdge(request: NewEdgeSnippetsPageRequest) {
    return this.http
      .post<{ result: GetEdgeSnippetsResult }>(
  ...
  getSnippetsForCluster(request: NewClusterSnippetsPageRequest) {
    return this.http
      .post<{ result: GetClusterSnippetsResult }>(
  ...
  getSnippetsForNodePair(
    ...
    return this.http
      .post<{ result: GetNodePairSnippetsResult }>(
