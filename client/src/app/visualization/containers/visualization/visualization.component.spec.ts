...
describe("VisualizationComponent", () => {
  ...
  configureTestSuite(() => {
    TestBed.configureTestingModule({
      ...
      declarations: [
        ...
        MockComponents(GraphSearchFormComponent, VisualizationCanvasComponent),
      ],
  ...
  beforeEach(() => {
    ...
    spyOn(legendService, "getAnnotationLegend").and.returnValue(
      of({
        gene: {
          color: "#673ab7",
          label: "gene",
    ...
    instance.networkGraphData =
      instance.setupInitialProperties(mockNeo4jResults);
  ...
  it("convertNodeToVisJSFormat should convert a graph node to vis js format", () => {
    ...
    expect(convertedMockNode).toEqual({
      ...
      label:
        mockGraphNode.displayName.length > 64
          ? mockGraphNode.displayName.slice(0, 64) + "..."
          : mockGraphNode.displayName,
    });
  });
  ...
  it("convertToVisJSFormat should convert neo4j query results to vis js format", () => {
    ...
    const convertedNeo4jResults =
      instance.convertToVisJSFormat(mockNeo4jResults);
  ...
  it("should emit getSnippetsForEdge subject when child requests snippets for edge", () => {
    const getSnippetsFromEdgeSpy = spyOn(
      instance.getEdgeSnippetsSubject,
      "next"
    );
    const visualizationCanvasComponentMock = fixture.debugElement.query(
      By.directive(VisualizationCanvasComponent)
    ...
    visualizationCanvasComponentMock.getSnippetsForEdge.emit(
      mockNewEdgeSnippetsPageRequest
    );

    expect(getSnippetsFromEdgeSpy).toHaveBeenCalledWith(
      mockNewEdgeSnippetsPageRequest
    );
  });

  it("should call getClusterData when child requests data for cluster", () => {
    const getClusterGraphDataSpy = spyOn(
      instance.getClusterSnippetsSubject,
      "next"
    );
    const visualizationCanvasComponentMock = fixture.debugElement.query(
      By.directive(VisualizationCanvasComponent)
    ...
    visualizationCanvasComponentMock.getSnippetsForCluster.emit(
      mockNewClusterSnippetsPageRequest
    );

    expect(getClusterGraphDataSpy).toHaveBeenCalledWith(
      mockNewClusterSnippetsPageRequest
    );
  });

  it("updateCanvasWithSingleNode should clear the canvas and add a single node", () => {
    instance.nodes = new DataSet<VisNode | GraphNode>(
      instance.networkGraphData.nodes
    );
    instance.edges = new DataSet<VisEdge | GraphRelationship>(
      instance.networkGraphData.edges
    );
