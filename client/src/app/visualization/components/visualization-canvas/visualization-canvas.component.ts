  ...
  implements OnInit, AfterViewInit
{
  ...
  @Output() getSnippetsForCluster =
    new EventEmitter<NewClusterSnippetsPageRequest>();
  ...
  @Input() set expandNodeResult(result: ExpandNodeResult) {
    try {
      if (!isNil(result)) {
        const edgeLabelsOfExpandedNode = this.getConnectedEdgeLabels(
          result.expandedNode
        );
        ...
        edgeLabelsOfExpandedNode.forEach((directionList, relationship) => {
          directionList.forEach((direction) => {
            ...
            duplicateNodeEdgePairs.forEach((pair) => {
              ...
              const edges = this.networkGraph.getConnectedEdges(
                duplicateNode.duplicateOf
              );
              ...
              if (edges.length === 1) {
                ...
                this.networkGraph.getConnectedNodes(duplicateNode.duplicateOf)
                  .length === 1
              ) {
    ...
    if (!isNil(result)) {
      try {
        ...
        if (isNil(toNode) || isNil(fromNode)) {
          throw Error(
            "One or more returned nodes do not exist on the network!"
          );
        }
  ...
  updateSettings(event: SettingsFormValues) {
    ...
    if (isNil(this.settingsFormValues)) {
          ...
          if (key === "maxClusterShownRows") {
            ...
            if (prevVal !== event[key].value) {
              this.clusters.forEach((_, clusterId) => {
                ...
                this.networkGraph.updateClusteredNode(clusterId, {
                  image: newClusterSvg,
                });
              });
            }
      ...
      this.networkGraph.setOptions({
        physics: this.settingsFormValues.animation.value,
      });
    }
  }
  ...
  collapseNeighbors(***ARANGO_USERNAME***Node: VisNode) {
    ...
    const connectedNodes = this.networkGraph.getConnectedNodes(
      ***ARANGO_USERNAME***Node.id
    ) as IdType[];

    // Remove every cluster connected to the ***ARANGO_USERNAME*** node
    ...
    this.edges.remove(
      this.networkGraph.getConnectedEdges(***ARANGO_USERNAME***Node.id) as IdType[]
    );
    ...
    const nodesToRemove = connectedNodes.map((connectedNodeId: number) => {
      if (this.networkGraph.findNode(connectedNodeId).length !== 0) {
        const connectedEdges =
          this.networkGraph.getConnectedEdges(connectedNodeId);
  ...
  getEdgesBetweenNodes(src: IdType, dest: IdType) {
    return this.networkGraph
      ...
      .filter((edge) =>
        this.networkGraph.getConnectedEdges(dest).includes(edge)
      );
  }
  ...
  isNotAClusterEdge(edge: IdType) {
    return (
      typeof edge !== "string" &&
      this.networkGraph.getClusteredEdges(edge).length === 1
    );
  }
    ...
    return this.networkGraph
      ...
      .map(
        (connectedEdgeWithRel) =>
          (
            this.networkGraph.getConnectedNodes(
              connectedEdgeWithRel
            ) as IdType[]
          ).filter((nodeId) => nodeId !== node)[0]
      );
  }
    ...
    this.networkGraph
      ...
      .forEach((edgeId) => {
        ...
        const connectedClustersOnRel = clustersConnectedToSelectedNode.filter(
          (clusterId: string) =>
            this.clusters.get(clusterId).relationship === label
        );
          ...
          const nonHubNodeIsInConnectedCluster = connectedClustersOnRel
            .map((clusterId) => {
              return this.networkGraph
                ...
                .map((duplicateNodeId) => {
                  return (this.nodes.get(duplicateNodeId) as DuplicateVisNode)
                    .duplicateOf;
                })
                .includes(nonHubNode);
            })
        ...
        if (!isNil(labels.get(label))) {
          ...
          const shouldAddTo =
            selectedNode === to && !labels.get(label).includes(Direction.TO);
          const shouldAddFrom =
            selectedNode === from &&
            !labels.get(label).includes(Direction.FROM);
          if (shouldAddTo || shouldAddFrom) {
            labels.set(label, [Direction.TO, Direction.FROM]);
          }
        } else {
          labels.set(label, [
            selectedNode === to ? Direction.TO : Direction.FROM,
          ]);
        }
      });
    return labels;
  ...
  createClusterSvg(referenceTableRows: ReferenceTableRow[]) {
    ...
    const numRowsToShow =
      referenceTableRows.length > maxRowsToShow
        ? maxRowsToShow
        : referenceTableRows.length;
    ...
    const rowsHTMLString = referenceTableRows
      ...
      .map((row, index) => {
        const percentOfMax =
          row.snippetCount === 0
            ? row.snippetCount
            : (row.snippetCount / maxSnippetCount) * 100;
        let rowHTMLString = `
    ...
    const longestName = referenceTableRows
      ...
      .sort(
        (a, b) =>
          ctx.measureText(b.nodeDisplayName).width -
          ctx.measureText(a.nodeDisplayName).width
      )[0].nodeDisplayName;
    ...
    const svgWidth = Math.max(
      ...
      Math.floor(
        ctx.measureText(maxNodesCellText).width * WIDTH_MULTIPLIER + FLUFF_WIDTH
      )
    );
  ...
  createDuplicateEdgeFromOriginal(
    ...
    return {
      ...
      from:
        originalEdge.from === clusterOrigin ? clusterOrigin : duplicateNode.id,
  ...
  cleanUpDuplicates(nodesInCluster: IdType[]) {
    ...
    nodesInCluster.forEach((duplicateNodeId) => {
      ...
      this.networkGraph
        ...
        .map(
          (duplicateEdgeId) =>
            this.edges.get(duplicateEdgeId) as DuplicateVisEdge
        )
        .forEach((duplicateEdge) => {
  ...
  createDuplicateNodesAndEdges(
    ...
    return neighborNodesWithRel.map((neighborNodeId) => {
      ...
      const newDuplicateNode = this.createDuplicateNodeFromOriginal(
  ...
  destroyCluster(clusterNodeId: IdType) {
    ...
    nodesInCluster.forEach((nodeId) => {
      this.networkGraph
        ...
        .forEach((edgeId) => edgesInCluster.push(edgeId));
    });
  ...
  createCluster(
    ...
    this.visService
      .getReferenceTableData(referenceTableDataRequest)
      .subscribe((result) => {
        ...
        this.networkGraph.getConnectedNodes(originNode).forEach((nodeId) => {
          if (this.networkGraph.isCluster(nodeId)) {
            ...
            if (
              cluster.relationship === relationship &&
        ...
        this.networkGraph.cluster({
          joinCondition: (n) =>
            duplicateNodeEdgePairs.map((pair) => pair.node.id).includes(n.id),
  ...
  groupNeighborsWithRelationship(groupRequest: GroupRequest) {
    ...
    try {
      const duplicateNodeEdgePairsOutsideCluster =
        this.createDuplicateNodesAndEdges(
          neighborNodesWithRel,
          relationship,
          node,
          direction
      ...
      duplicateNodeEdgePairs = duplicateNodeEdgePairs.concat(
        duplicateNodeEdgePairsOutsideCluster
      );
      ...
      this.networkGraph.getConnectedNodes(node).forEach((nodeId) => {
        if (this.networkGraph.isCluster(nodeId)) {
          if (this.clusters.get(nodeId).relationship === relationship) {
            ...
            const duplicateNodesEdgePairsInCluster =
              this.getDuplicateNodeEdgePairsFromCluster(nodeId).filter(
                // It is possible that some of the nodes inside the cluster are also outside it. So,
                // get rid of duplicates.
                (pair: DuplicateNodeEdgePair) =>
                  !neighborNodesWithRel.includes(pair.node.duplicateOf)
              );
            duplicateNodeEdgePairs = duplicateNodeEdgePairs.concat(
  ...
  selectNeighbors(node: IdType) {
    this.networkGraph.selectNodes(
      this.networkGraph.getConnectedNodes(node) as IdType[]
    );
    this.updateSelectedNodes();
  }
  ...
  openTypeSidenav(type: AssociatedType) {
    ...
    if (
      ...
      this.networkGraph
        .getConnectedNodes(sourceNode.id)
        .forEach((connectedNodeId) => {
          if (!this.networkGraph.isCluster(connectedNodeId)) {
            ...
            const knowledgeGraphId =
              connectedNode.duplicateOf || connectedNode.id;
            ...
            if (
              ...
              if (!isNil(connectedNode.duplicateOf)) {
                connectedNodes.push(
                  this.createOriginalNodeFromDuplicate(connectedNode)
                );
              } else {
  ...
  onContextCallback(params: any) {
    const hoveredNode = this.networkGraph.getNodeAt(
      params.pointer.DOM
    ) as string;
    ...
    this.contextMenuControlService.updatePopper(
      contextMenuXPos,
      contextMenuYPos
    );
    ...
    if (hoveredNode !== undefined) {
      if (
        ...
        !currentlySelectedNodes.includes(hoveredNode)
      ) {
        this.networkGraph.selectNodes([hoveredNode], false);
      }
    } else if (hoveredEdge !== undefined) {
      if (
        currentlySelectedEdges.length === 0 ||
        !currentlySelectedEdges.includes(hoveredEdge)
      ) {
