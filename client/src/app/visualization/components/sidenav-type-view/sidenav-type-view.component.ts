...
export class SidenavTypeViewComponent {
  ...
  updateNodeEntity(nodeEntity: SidenavTypeEntity) {
    ...
    this.visualizationService.getAssociatedTypeSnippetCount(request).subscribe(
      (associatedTypes) => {
        this.typeEntries = [];
        const max =
          associatedTypes.length > 0 ? associatedTypes[0].snippetCount : 0;
  ...
  getSnippets(node1Id: number, node2Id: number, limit: number, page: number) {
    this.visualizationService
      .getSnippetsForNodePair(node1Id, node2Id, page, limit)
      .subscribe(
        (result) => {
          ...
          this.selectedRowSnippetData = result.snippetData.map((row) => {
            ...
            if (row.fromNodeId === this.node.id) {
              fromNode = this.node;
              toNode = this.connectedNodes.find(
                (node) => node.id === row.toNodeId
              );
            } else {
              fromNode = this.connectedNodes.find(
                (node) => node.id === row.fromNodeId
              );
              toNode = this.node;
            }
