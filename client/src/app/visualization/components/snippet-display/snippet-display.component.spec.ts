...
describe("SnippetDisplayComponentComponent", () => {
  ...
  it("should load snippet panels", () => {
    const snippetPanels = document.getElementsByClassName(
      "association-snippet-panel"
    );
  ...
  it("should show publication data on snippet panels", () => {
    const snippetPanelTitles = document.getElementsByClassName(
      "snippet-panel-title"
    );
    const snippetPanelPubData = document.getElementsByClassName(
      "snippet-panel-pub-data"
    );
  ...
  it("should link to pubtator", () => {
    const snippetPanel = document.getElementsByClassName(
      "snippet-panel-title"
    )[0] as HTMLElement;
  ...
  it('should show "Showing 0 - 0" of 0" and no page limit selector if there are no results', () => {
    const resultsDisplayContainer = document.getElementById(
      "num-snippets-container"
    );
    component.totalResults = 0;

    fixture.detectChanges();

    expect(resultsDisplayContainer.innerText).toEqual(
      "Showing 0 - 0 of 0 snippets"
    );
  });
  ...
  it("requestPage should request new data from the parent for selected page", () => {
    const requestNewPageEmitterSpy = spyOn(
      component.requestNewPageEmitter,
      "emit"
    );
