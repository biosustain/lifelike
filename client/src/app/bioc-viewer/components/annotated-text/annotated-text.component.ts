      ...
      (acc, annotation) => {
        return annotation.locations.reduce((iacc, location) => {
          ...
          for (idx = 0; idx < iacc.length; idx++) {
            part = iacc[idx];
            const localOffset = part.location
              ? part.location.length
              : part.length;
            ...
            if (part.location) {
              ...
              offset =
                part.location.offset + part.location.length - globalOffset;
            } else {
              offset += localOffset;
            }
          }
          ...
          if (part.slice(startOffset, endOffset) !== annotation.text) {
            for (let i = 0; i < part.length; i++) {
              if (
                part.slice(startOffset - i, endOffset - i) === annotation.text
              ) {
                break;
              }
              if (
