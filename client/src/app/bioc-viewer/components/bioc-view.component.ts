...
export class BiocViewComponent implements OnDestroy, ModuleAwareComponent {
  constructor(
    ...
    this.openbiocSub = this.loadTask.results$.subscribe(
      ({ result: [object, content], value: [file] }) => {
        ...
        const ref = firstDoc.passages.findIndex(
          (p) => p.infons.section_type === "REF"
        );
        if (ref > -1) {
  ...
  searchChanged: Subject<{ keyword: string; findPrevious: boolean }> =
    new Subject<{ keyword: string; findPrevious: boolean }>();
  searchQuery = "";
  ...
  pmid(doc) {
    ...
    if (pmid) {
      const PMID_LINK =
        ...
        String(pmid.infons["article-id_pmid"]);
    ...
    if (pmc) {
      const PMCID_LINK =
        ...
        String(pmc.infons["article-id_pmc"]);
  ...
  title(doc) {
    try {
      return doc.passages.find(
        (p) =>
          p.infons.type.toLowerCase() === "title" ||
          p.infons.section_type.toLowerCase() === "title"
      ).text;
    } catch (e) {
      return doc.pmid;
    }
  ...
  scrollInOffset(params: BiocAnnotationLocation) {
    if (!isNaN(Number(params.offset))) {
      ...
      const annotationElem =
        this._elemenetRef.nativeElement.querySelector(query);
      if (annotationElem) {
    ...
    if (params.start && params.len) {
      ...
      range.setEnd(
        annotationElem.firstChild,
        Number(params.start) + Number(params.len)
      );
  ...
  scrollInbioc(loc: Location) {
    ...
    if (!this.biocFileLoaded) {
      console.log(
        "File in the bioc viewer is not loaded yet. So, I cant scroll"
      );
      return;
    }
  ...
  openFileNavigatorPane() {
    ...
    this.workSpaceManager.navigateByUrl({
      url,
      extras: { sideBySide: true, newTab: true },
    });
  }
  ...
  dragStart(event: DragEvent) {
    ...
    const id = ((event.target as Element).attributes[`identifier`] || {})
      .nodeValue;
    const annType = ((event.target as Element).attributes[`annType`] || {})
      .nodeValue;
    const src = this.getSource({
      identifier: id,
      type: annType,
    });
    const offset = ((event.target as Element).attributes[`offset`] || {})
      .nodeValue;
    const search = [];
    const hyperlinks = [];
    const url = src;
    ...
    dataTransfer.setData(
      ...
      JSON.stringify({
        display_name: txt,
        label:
          String(type).toLowerCase() === "text-truncate"
            ? "link"
  ...
  getSource(payload: Infon = {}) {
    ...
    const fallback = SEARCH_LINKS.find(
      (a) => a.domain.toLowerCase() === "google"
    );
