  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  constructor(
    ...
    this.findController = new AsyncElementFind(
      ...
      this.annotation.id.length
        ? this.generateAnnotationFindQueue
        : this.generateTextFindQueue
    );
    this.findController.query = this.annotation.id.length
      ? this.annotation.id
  ...
  load() {
    ...
    this.ngZone.runOutsideAngular(
      () =>
        (this.tickAnimationFrameId = requestAnimationFrame(
          this.tick.bind(this)
        ))
    );
  }
      ...
      next: () => {
        ...
        if (this.findTarget.first && this.findController.target === null) {
          this.findController.target =
            this.findTarget.first.nativeElement.getElementsByTagName(
              "tbody"
            )[0];
  ...
  save() {
    ...
    observable.subscribe(() => {
      this.snackBar.open(`Enrichment table saved.`, "Close", {
        duration: 5000,
      });
    });
          ...
          if (document.domains !== result) {
            ...
            this.table$ = new EnrichmentTable().load(document).pipe(
              this.errorHandler.create({
                label: "Re-order enrichment table",
              }),
              shareReplay()
            );
          }
        ...
        case Node.ELEMENT_NODE:
          ...
          if (style.display === "block") {
            ...
            while (match !== null) {
              ...
              const lastIndexIsEOS =
                regex.lastIndex === node.textContent.length;
              const endOfMatch = lastIndexIsEOS
                ? regex.lastIndex - 1
                : regex.lastIndex;
