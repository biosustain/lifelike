...
} from "../models/enrichment-document";
...
export class EnrichmentVisualisationService {
  ...
  load: Observable<
    [TaskResult<null, FilesystemObject>, TaskResult<null, EnrichmentParsedData>]
  >;
  loaded = false;
  enrichmentDocument: BaseEnrichmentDocument;

  set fileId(fileId: string) {
    const enrichmentDocument = (this.enrichmentDocument =
      new BaseEnrichmentDocument());
