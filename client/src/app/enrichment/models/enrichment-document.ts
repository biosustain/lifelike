...
export class BaseEnrichmentDocument {
  ...
  parseParameters({
    ...
    const rawImportGenes = importGenes
      .map((gene) => gene.trim())
      .filter((gene) => gene !== "");
  ...
  decode({ data, result, ...rest }: EnrichmentData): EnrichmentParsedData {
    ...
    const domains = data.sources.filter(
      (domain) =>
        domain.length && (domain !== "KEGG" || environment.keggEnabled)
    );
...
export class EnrichmentDocument extends BaseEnrichmentDocument {
  ...
  refreshData(): Observable<this> {
    ...
    if (this.fileId === "") {
      ...
      return this.generateEnrichmentResults(
        this.domains,
        this.importGenes,
        this.taxID
      ).pipe(
    ...
    return this.worksheetViewerService.getAnnotatedEnrichment(this.fileId).pipe(
      mergeMap((enriched: EnrichmentParsedData) => {
        if (Object.keys(enriched).length > 0) {
          ...
          return this.worksheetViewerService
            .annotateEnrichment([this.fileId], params)
            .pipe(
              mergeMap(() =>
                this.worksheetViewerService
                  .getAnnotatedEnrichment(this.fileId)
                  .pipe(
        ...
        return this.worksheetViewerService
          ...
          .pipe(
            map((domainResults: EnrichmentWrapper): EnrichmentResult => {
              ...
              const neo4jIdSynonymMap: Map<
                number,
                Map<number, string>
              > = new Map();
              const neo4jIdNodeMap: Map<
                number,
                Map<number, NCBINode>
              > = new Map();
              const neo4jIdLinkMap: Map<
                number,
                Map<number, string>
              > = new Map();
              const geneSynonymNeo4jIdMap: Map<
                number,
                Map<number, number>
              > = new Map();
              ...
              ncbiNodesData.forEach((wrapper) => {
                ...
                neo4jIdNodeMap.has(wrapper.synonymNeo4jId)
                  ? neo4jIdNodeMap
                      .get(wrapper.synonymNeo4jId)
                      .set(wrapper.geneNeo4jId, wrapper.gene)
                  : neo4jIdNodeMap.set(
                      wrapper.synonymNeo4jId,
                ...
                neo4jIdLinkMap.has(wrapper.synonymNeo4jId)
                  ? neo4jIdLinkMap
                      .get(wrapper.synonymNeo4jId)
                      .set(wrapper.geneNeo4jId, wrapper.link)
                  : neo4jIdLinkMap.set(
                      wrapper.synonymNeo4jId,
              ...
              for (const [synId, geneId] of synonymNeo4jIds) {
                ...
                const domainWrapper =
                  domainResults[geneSynonymNeo4jIdMap.get(synId).get(geneId)] ||
                  null;
                ...
                if (domainWrapper !== null) {
                  ...
                  genesList.push({
                    ...
                    link,
                    domains: this.generateGeneDomainResults(
                      domains,
                      domainWrapper,
                      node
                    ),
