...
> extends AbstractCanvasBehavior {
  ...
  drag(event: DragBehaviorEvent): BehaviorResult {
    if (this.handle) {
      const transform = this.graphView.transform;
      const [mouseX, mouseY] = d3.mouse(this.graphView.canvas);
      this.activeDrag(event.event, {
        x: transform.invertX(mouseX),
        y: transform.invertY(mouseY),
      });
  ...
  protected getNodeSize(node: UniversalGraphNode): {
    width: number;
    height: number;
  } {
  ...
  draw(ctx: CanvasRenderingContext2D, transform: ZoomTransform) {
    ...
    for (const handle of Object.values(
      this.getHandleBoundingBoxes(placedNode)
    )) {
