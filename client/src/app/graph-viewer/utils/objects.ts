...
import {
  ...
  UniversalGraphGroup,
  UniversalGraphImageNodeTemplate,
  UniversalGraphNode,
 ...
 */
export const createNode = <N extends Partial<UniversalGraphNodeTemplate>>(partialNode: N) =>
  merge(NODE_DEFAULTS_FACTORY(), cloneDeep(partialNode), { hash: uuidv4() }) as ReturnType<
    typeof NODE_DEFAULTS_FACTORY
  > &
    N &
    Pick<UniversalGraphNode, 'hash'>;
 ...
 */
export const createGroupNode = <G extends Partial<UniversalGraphGroupTemplate>>({
  members = [],
  ...partialGroupRest
}: G) =>
  assign(
    createNode(merge(GROUP_DEFAULTS_FACTORY(), partialGroupRest)),
    { members } // Members are passed by reference (cannot be cloned)
  ) as ReturnType<typeof NODE_DEFAULTS_FACTORY> &
    ReturnType<typeof GROUP_DEFAULTS_FACTORY> &
    G &
    Pick<UniversalGraphNode, 'hash'>;
 ...
 */
export const createImageNode = <IN extends Partial<UniversalGraphImageNodeTemplate>>(
  partialImage: IN
) =>
  createNode(merge(IMAGE_DEFAULTS_FACTORY(), partialImage, { image_id: makeid() })) as ReturnType<
    typeof NODE_DEFAULTS_FACTORY
  > &
    ReturnType<typeof IMAGE_DEFAULTS_FACTORY> &
    IN &
    Pick<UniversalGraphNode, 'hash'> &
    Pick<UniversalGraphGroup, 'image_id'>;
