      ...
      ([newSelection, oldSelection]) => {
        if (
          ...
          this.graphView.behaviors.add(
            BEHAVIOR_KEY,
            new ActiveNodeResize(this.graphView, newSelection[0].entity as UniversalGraphNode),
            100
          );
          ...
          this.graphView.behaviors.add(
            BEHAVIOR_KEY,
            new ActiveGroupResize(this.graphView, newSelection[0].entity as UniversalGraphGroup),
            100
          );
        } else {
          this.graphView.behaviors.delete(BEHAVIOR_KEY);
...
export class ActiveNodeResize extends ActiveResize<UniversalGraphNode> {
  getHandleBoundingBoxes(placedNode: PlacedNode): DragHandle[] {
    ...
    if (placedNode.uniformlyResizable) {
      ...
      const execute = (target, originalData: OriginalData, dragStartPosition, graphPosition) => {
        ...
        const sizingVecLen =
          Math.hypot(graphPosition.x - x, graphPosition.y - y) - handleDiagonal / 2;
  ...
  protected activeDragEnd(event: MouseEvent) {
    if (
      this.target.data.width !== this.originalTarget.data.width ||
...
export class ActiveGroupResize extends ActiveResize<UniversalGraphGroup> {
  getHandleBoundingBoxes(placedGroup: PlacedGroup): DragHandle[] {
    ...
    const scaleMemberFactory =
      (scale, originalTargetData, targetData, internalSize, centerAccessor, sizeAccessor) =>
      ([targetGroupMember, originalGroupMember]) => {
        const originalMemberSize =
          originalGroupMember.data[sizeAccessor] ||
          this.getNodeSize(originalGroupMember)[sizeAccessor];
        const scalledEdges = centric2edge([
          originalGroupMember.data[centerAccessor],
          originalMemberSize,
        ...
        if (originalMemberSize < internalSize) {
          if (originalGroupMember.data[centerAccessor] < originalTargetData[centerAccessor]) {
            ...
            originalGroupMember.data[centerAccessor] > originalTargetData[centerAccessor]
          ) {
          ...
          [targetGroupMember.data[centerAccessor], targetGroupMember.data[sizeAccessor]] =
            edge2centric(scalledEdges);
        } else {
          [targetGroupMember.data[centerAccessor], targetGroupMember.data[sizeAccessor]] = [
            targetData[centerAccessor],
            internalSize,
          ];
        }
      };

    const handleExecuteFactory =
      (edge2grab: 0 | 1, centerAccessor: "x" | "y", sizeAccessor: "width" | "height") =>
      (target, originalData, dragStartPosition, graphPosition) => {
        const {
        ...
        const distance =
          (graphPosition[centerAccessor] - this.dragStartPosition[centerAccessor]) * noZoomScale;
        const orgEdges = centric2edge([
          originalTargetData[centerAccessor],
          originalTargetData[sizeAccessor],
        ]);
        const scale = scaleLinear()
          ...
          .clamp(true);
        [targetData[centerAccessor], targetData[sizeAccessor]] = edge2centric(
          orgEdges.map(scale) as [number, number]
        );
        zip(this.target.members, this.originalTarget.members).forEach(
          scaleMemberFactory(
            scale,
            originalTargetData,
            targetData,
            targetData[sizeAccessor] - margin * 2,
            centerAccessor,
            sizeAccessor
          )
        );
    ...
    const execute = (target, originalData: OriginalData, dragStartPosition, graphPosition) => {
      ...
      const sizingVecLen =
        Math.hypot(graphPosition.x - originalData.x, graphPosition.y - originalData.y) -
        handleDiagonal / 2;
      const normY = Math.abs(sizingVecLen / Math.sqrt(ratio ** 2 + 1));
      ...
      const scaleY = scaleLinear().domain(orgYEdges).range(targetYEdges).clamp(true);
      ...
      const scaleX = scaleLinear().domain(orgXEdges).range(targetXEdges).clamp(true);

      const scaleXMember = scaleMemberFactory(
        scaleX,
        originalData,
        target.data,
        target.data.width - target.margin * 2,
        "x",
        "width"
      );
      const scaleYMember = scaleMemberFactory(
        scaleY,
        originalData,
        target.data,
        target.data.height - target.margin * 2,
        "y",
        "height"
      );
      zip(this.target.members, this.originalTarget.members).forEach((d) => {
        scaleXMember(d);
        scaleYMember(d);
      });
    };
  ...
  protected activeDragEnd(event: MouseEvent) {
    ...
    zip(this.originalTarget.members, this.target.members).forEach(
      ([originalGroupMember, targetGroupMember]) => {
        actions.push(
          new GraphEntityUpdate(
            "Node resize from group resize",
            {
              type: GraphEntityType.Node,
