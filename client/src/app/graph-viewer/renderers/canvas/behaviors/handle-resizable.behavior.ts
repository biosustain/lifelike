...
export class HandleResizableBehavior extends AbstractCanvasBehavior {
  ...
  setup() {
    this.selectionChangeSubscription =
      this.graphView.selection.changeObservable.subscribe(
        ([newSelection, oldSelection]) => {
          if (
            newSelection.length === 1 &&
            newSelection[0].type === GraphEntityType.Node &&
            this.graphView.placeNode(
              newSelection[0].entity as UniversalGraphNode
            ).resizable
          ) {
            this.graphView.behaviors.delete(BEHAVIOR_KEY);
            this.graphView.behaviors.add(
              BEHAVIOR_KEY,
              new ActiveNodeResize(
                this.graphView,
                newSelection[0].entity as UniversalGraphNode
              ),
              100
            ...
            this.graphView.behaviors.add(
              BEHAVIOR_KEY,
              new ActiveGroupResize(
                this.graphView,
                newSelection[0].entity as UniversalGraphGroup
              ),
              100
            );
          } else {
...
> extends AbstractObjectHandleBehavior<DragHandle, Target> {
  ...
  isPointIntersectingNodeHandles(
    ...
    return !!this.handle || !!this.getHandleIntersected(placedObject, point)
      ? true
      : undefined;
  }
  ...
  getHandleBoundingBoxes(placedNode: PlacedNode): DragHandle[] {
    ...
    const handles = [
      ...
      this.sideHandleMaker(
        ...
        (target, originalData, dragStartPosition, graphPosition) => {
          const distance =
            (graphPosition.x - this.dragStartPosition.x) * noZoomScale;
      ...
      this.sideHandleMaker(
        ...
        (target, originalData, dragStartPosition, graphPosition) => {
          const distance =
            (graphPosition.y - this.dragStartPosition.y) * noZoomScale;
    ...
    if (placedNode.uniformlyResizable) {
      ...
      const execute = (
        ...
        const sizingVecLen =
          Math.hypot(graphPosition.x - x, graphPosition.y - y) -
          handleDiagonal / 2;
  ...
  getHandleBoundingBoxes(placedGroup: PlacedGroup): DragHandle[] {
    ...
    const scaleMemberFactory =
      ...
      ([targetGroupMember, originalGroupMember]) => {
        ...
        if (originalMemberSize < internalSize) {
          ...
          [
            targetGroupMember.data[centerAccessor],
            targetGroupMember.data[sizeAccessor],
          ] = edge2centric(scalledEdges);
        } else {
          [
            targetGroupMember.data[centerAccessor],
            targetGroupMember.data[sizeAccessor],
          ] = [targetData[centerAccessor], internalSize];
        }
      };
    ...
    const handleExecuteFactory =
      ...
      (target, originalData, dragStartPosition, graphPosition) => {
        ...
        const distance =
          (graphPosition[centerAccessor] -
            this.dragStartPosition[centerAccessor]) *
          noZoomScale;
    ...
    const execute = (
      ...
      const sizingVecLen =
        Math.hypot(
          graphPosition.x - originalData.x,
          graphPosition.y - originalData.y
        ) -
        handleDiagonal / 2;
      const normY = Math.abs(sizingVecLen / Math.sqrt(ratio ** 2 + 1));
