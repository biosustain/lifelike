    ...
    if (position) {
      ...
      if (content) {
        ...
        this.graphView.execute(
          this.createActionFromPasteContent(content, position)
        );
    ...
    try {
      ...
      if (type === TYPE_STRING) {
        const centerOfMass = _flow(
          ...
          _flatMap(
            ({ entity }) =>
              ((entity as UniversalGraphGroup).members ?? [
                entity,
        ...
        const pasteNode = <N extends UniversalGraphNode>({
          hash,
          ...rest
        }: N) => {
        ...
        for (const {
          ...
          members,
          ...rest
        } of groups as UniversalGraphGroup[]) {
