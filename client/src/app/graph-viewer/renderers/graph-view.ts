...
export abstract class GraphView<BT extends Behavior> implements GraphActionReceiver {
  ...
  setGraph(graph: KnowledgeMapGraph): void {
    ...
    const nodelikeMap = graph.nodes.reduce((map, node) => map.set(node.hash, node), new Map());
  ...
  removeNodeLike<N extends UniversalGraphNodelike>(
    ...
    const removedEdges = remove(
      this.edges,
      (edge) =>
        this.expectNodelikeByHash(edge.from) === node || this.expectNodelikeByHash(edge.to) === node
    );

    this.nodelikeHashMap.delete(node.hash);
    (node as UniversalGraphGroup).members?.forEach((m) => this.groupHashMap.delete(m.hash));
