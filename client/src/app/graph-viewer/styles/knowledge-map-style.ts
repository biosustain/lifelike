...
import { AppURL, getInternalURIType } from 'app/shared/url/url';
...
export class KnowledgeMapStyle implements NodeRenderStyle, EdgeRenderStyle, GroupRenderStyle {
  ...
  placeNode(
    ...
    const strokeColor =
      styleData.strokeColor ?? (annotationStyle?.style?.border || BORDER_BLUE_COLOR);
   ...
   */
  private createLine(type: string | undefined, width: number, style: string): Line | undefined {
    if (type == null) {
      return null;
    }
