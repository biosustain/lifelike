...
var Rule = /** @class */ (function (_super) {
  ...
  Rule.metadata = {
    description:
      "Disallow passing through ***ARANGO_USERNAME*** of module with relative imports",
    options: [],
    optionsDescription: "",
  ...
  Rule.FAILURE_STRING =
    "Root of application cannot be imported with relative path";
  return Rule;
})(Lint.Rules.AbstractRule);
exports.Rule = Rule;
function walk(ctx) {
  for (
    var _i = 0, _a = tsutils_1.findImports(ctx.sourceFile, 63 /* All */);
    _i < _a.length;
    _i++
  ) {
    var name_1 = _a[_i];
    if (typescript_1.isExternalModuleNameRelative(name_1.text)) {
      ...
      var relativeRoot = path.resolve.apply(
        path,
        __spreadArrays([sourceDir], relativePart)
      );
      // '.' workspace
      ...
      if (relative.length === 2 && relative[1] === "app") {
        ...
        var fix = new Lint.Replacement(
          name_1.getStart(),
          name_1.getWidth(),
          "'" + __spreadArrays(["app"], remainingPart).join("/") + "'"
        );
        ctx.addFailure(
          name_1.getStart(ctx.sourceFile) + 1,
          name_1.end - 1,
          Rule.FAILURE_STRING,
          fix
        );
